/* MAHPLAZA v5 (ULTRA CHROMIUM CITY closure) — actual-runtime review set.
   Same page a phone opens (mahworld/scene/mahplaza.html), served from the working tree to
   headless Chromium (software GL). Writes validation/mahworld/mahplaza-v5/:
     ...
     review    Review.html (static, relative assets, BASELINE = v3 / AFTER = v5, REFERENCE = concept)
   Run: node tests/mahworld-mahplaza-capture-v5.js [core phone noui life quality review]  (no args = all) */
'use strict';
const fs = require('fs'), p = require('path'), cp = require('child_process');
const ROOT = p.resolve(__dirname, '..');
const OUT = p.join(ROOT, 'validation/mahworld/mahplaza-v5');
const BASE = p.join(ROOT, 'validation/mahworld/mahplaza-v4');
...
if (!pw) { console.log('mahworld-mahplaza-capture-v5: SKIP (playwright is not installed)'); process.exit(0); }
const { chromium } = pw;
...
const BASELINE = ['establishing', 'in-world', 'skyline', 'plaza-node', 'gym-entrance', 'match-entrance', 'match-hall', 'market-entrance', 'residents', 'sky-plant', 'appearance']
  .reduce((o, v) => { o[v] = `v4-${v}-night-wide.png`; return o; }, {});
...
(async () => {
  ...
  if (want('core')) {
    ...
    for (const time of ['night', 'day']) { await setTime(page, time); for (const [view, title] of VIEWS) { await setView(page, view); await shot(page, `v5-${view}-${time}-wide.png`, { set: 'core', view, time, label: 'wide', title }); } }
    await setTime(page, 'dusk'); for (const view of ['establishing', 'skyline']) { await setView(page, view); await shot(page, `v5-${view}-dusk-wide.png`, { set: 'core', view, time: 'dusk', label: 'wide' }); }
  ...
  if (want('phone')) {
    ...
    for (const view of ['establishing', 'in-world', 'skyline', 'match-entrance', 'match-hall', 'residents']) { await setView(page, view); await shot(page, `v5-${view}-night-phone.png`, { set: 'phone', view, time: 'night', label: 'phone' }); }
    await setTime(page, 'day'); for (const view of ['in-world', 'match-hall']) { await setView(page, view); await shot(page, `v5-${view}-day-phone.png`, { set: 'phone', view, time: 'day', label: 'phone' }); }
    await ctx.close();
    ...
    await shot(page2, 'v5-in-world-night-phone-with-controls.png', { set: 'phone', view: 'in-world', time: 'night', label: 'phone with controls' });
  ...
  if (want('noui')) {
    ...
    for (const view of ['in-world', 'establishing', 'skyline', 'match-hall']) { await setView(page, view); await shot(page, `v5-noui-${view}-night.png`, { set: 'noui', view, time: 'night', label: 'no labels' }); }
    await ctx.close(); save();
  }
  ...
  if (want('life')) {
    ...
    for (const at of [0, 20, 40, 60]) {
      ...
      const f = `v5-life-in-world-${at}s.png`; await page.screenshot({ path: p.join(OUT, f) });
    ...
    for (let i = 0; i < 40 && !framed; i++) {
      ...
      const f = 'v5-life-event.png'; await page.screenshot({ path: p.join(OUT, f) });
  ...
  if (want('quality')) {
    ...
    await setView(page, 'in-world');
    report.quality = [];
    for (const q of ['high', 'medium', 'low']) { await page.evaluate(q => window.MAHWORLD_MAHPLAZA.setQuality(q), q); await page.waitForTimeout(1500); const f = `v5-quality-${q}-in-world-night.png`; await page.screenshot({ path: p.join(OUT, f) }); const i = await page.evaluate(info); report.quality.push(Object.assign({ file: f, tier: q }, i)); console.log('quality', q, 'draws', i.drawCalls, 'tris', i.triangles, 'ms', i.ms, 'casters', i.shadowCasters); }
    await ctx.close(); save();
  }
  await browser.close(); save();
  if (want('review')) writeReview();
  console.log('mahworld-mahplaza-capture-v5: ' + (report.captures || []).length + ' captures, page errors ' + report.errors.length + (report.logs.length ? ' | ' + report.logs.slice(0, 4).join(' | ') : ''));
...
function writeReview() {
  ...
  const changed = (() => { try { return cp.execSync('git diff --stat 35a1219 -- . ":(exclude)validation"', { cwd: ROOT }).toString().trim(); } catch (e) { return ''; } })();
  const refs = fs.existsSync(p.join(BASE, 'reference')) ? fs.readdirSync(p.join(BASE, 'reference')).map(f => '../mahplaza-v4/reference/' + f) : [];
  const aaaRef = fs.existsSync(p.join(OUT, 'reference')) ? fs.readdirSync(p.join(OUT, 'reference')).map(f => 'reference/' + f) : [];
  let html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MAHPLAZA v5 — chromium city review</title>
...
<p class="lead">Actual-runtime captures from <code>mahworld/scene/mahplaza.html</code> on the permitted renderer. Revision <code>${esc(report.revision)}</code>, captured ${esc(report.ranAt)}, ${esc(report.chromium || '')} with software GL. BASELINE frames are the v4 crystalline-night pass (<code>../mahplaza-v4/</code>); AFTER frames are this pass. Modules present at capture: <code>${esc(JSON.stringify(report.modules || {}))}</code>.</p>`;
  ...
  for (const [view, title] of VIEWS) { const b = BASELINE[view], c = cap(view, 'night', 'wide'); if (!c) continue; if (b && fs.existsSync(p.join(BASE, b))) html += fig('../mahplaza-v4/' + b, 'v4 — ' + title + (baseInfo(view) ? ' — ' + baseInfo(view).drawCalls + ' draws · ' + baseInfo(view).triangles.toLocaleString() + ' tris' : ''), 'BASELINE'); html += fig(c.file, 'v5 — ' + title + ' — ' + meta(c), 'AFTER'); }
...
<p>Baseline v4 (night, wide): ${esc(baseCap.filter(c => c.label === 'wide' && c.time === 'night').map(c => c.view + ' ' + c.drawCalls + ' draws / ' + c.triangles.toLocaleString() + ' tris').join(' · '))}</p>
<h3>Changed files against 35a1219 (v4)</h3><pre>${esc(changed)}</pre>
