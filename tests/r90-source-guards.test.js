...
const checks=[
  ...
  [server.includes('const V = 447;')&&sw.includes("fob-shell-v447"),'R90 cache identity advances canonical role-context assets']
];
let passed=0;for(const [ok,msg] of checks){assert.ok(ok,msg);passed++}console.log('r90-source-guards: '+passed+'/'+checks.length+' PASS');
