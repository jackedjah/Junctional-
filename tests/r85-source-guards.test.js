...
const checks=[
  ...
  [server.includes('const V = 449;')&&sw.includes("fob-shell-v449"),'R90 cache identity advances canonical role-context assets']
];
let passed=0;for(const [ok,msg] of checks){assert.ok(ok,msg);passed++}console.log('r85-source-guards: '+passed+'/'+checks.length+' PASS');
