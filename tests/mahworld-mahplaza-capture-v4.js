/* MAHPLAZA v4 (AAA closure pass) — actual-runtime review set.
   Same page a phone opens (mahworld/scene/mahplaza.html), served from the working tree to
   headless Chromium (software GL). Writes validation/mahworld/mahplaza-v4/:
     ...
     review    Review.html (static, relative assets, BASELINE = v3 / AFTER = v4, REFERENCE = concept)
   Run: node tests/mahworld-mahplaza-capture-v4.js [core phone noui life quality review]  (no args = all) */
'use strict';
const fs = require('fs'), p = require('path'), cp = require('child_process');
const ROOT = p.resolve(__dirname, '..');
const OUT = p.join(ROOT, 'validation/mahworld/mahplaza-v4');
const BASE = p.join(ROOT, 'validation/mahworld/mahplaza-v3');
...
if (!pw) { console.log('mahworld-mahplaza-capture-v4: SKIP (playwright is not installed)'); process.exit(0); }
const { chromium } = pw;
...
const BASELINE = { establishing: 'v3-establishing-night-wide.png', 'in-world': 'v3-in-world-night-wide.png', 'gym-entrance': 'v3-gym-entrance-night-wide.png', 'match-entrance': 'v3-match-entrance-night-wide.png', 'market-entrance': 'v3-market-entrance-night-wide.png', residents: 'v3-residents-night-wide.png', 'sky-plant': 'v3-sky-plant-night-wide.png', appearance: 'v3-check-1-blue-world-purple-self-wide.png' };
...
(async () => {
  ...
  if (want('core')) {
    ...
    for (const time of ['night', 'day']) { await setTime(page, time); for (const [view, title] of VIEWS) { await setView(page, view); await shot(page, `v4-${view}-${time}-wide.png`, { set: 'core', view, time, label: 'wide', title }); } }
    await setTime(page, 'dusk'); for (const view of ['establishing', 'skyline']) { await setView(page, view); await shot(page, `v4-${view}-dusk-wide.png`, { set: 'core', view, time: 'dusk', label: 'wide' }); }
  ...
  if (want('phone')) {
    ...
    for (const view of ['establishing', 'in-world', 'skyline', 'match-entrance', 'match-hall', 'residents']) { await setView(page, view); await shot(page, `v4-${view}-night-phone.png`, { set: 'phone', view, time: 'night', label: 'phone' }); }
    await setTime(page, 'day'); for (const view of ['in-world', 'match-hall']) { await setView(page, view); await shot(page, `v4-${view}-day-phone.png`, { set: 'phone', view, time: 'day', label: 'phone' }); }
    await ctx.close();
    ...
    await shot(page2, 'v4-in-world-night-phone-with-controls.png', { set: 'phone', view: 'in-world', time: 'night', label: 'phone with controls' });
  ...
  if (want('quality')) {
    ...
    await setView(page, 'in-world');
    report.quality = [];
    for (const q of ['high', 'medium', 'low']) { await page.evaluate(q => window.MAHWORLD_MAHPLAZA.setQuality(q), q); await page.waitForTimeout(1500); const f = `v4-quality-${q}-in-world-night.png`; await page.screenshot({ path: p.join(OUT, f) }); const i = await page.evaluate(info); report.quality.push(Object.assign({ file: f, tier: q }, i)); console.log('quality', q, 'draws', i.drawCalls, 'tris', i.triangles, 'ms', i.ms, 'casters', i.shadowCasters); }
    await ctx.close(); save();
  }
  await browser.close(); save();
  if (want('review')) writeReview();
  console.log('mahworld-mahplaza-capture-v4: ' + (report.captures || []).length + ' captures, page errors ' + report.errors.length + (report.logs.length ? ' | ' + report.logs.slice(0, 4).join(' | ') : ''));
...
function writeReview() {
  ...
  const changed = (() => { try { return cp.execSync('git diff --stat 068fea2 -- . ":(exclude)validation"', { cwd: ROOT }).toString().trim(); } catch (e) { return ''; } })();
  const refs = fs.existsSync(p.join(BASE, 'reference')) ? fs.readdirSync(p.join(BASE, 'reference')).map(f => '../mahplaza-v3/reference/' + f) : [];
  const aaaRef = fs.existsSync(p.join(OUT, 'reference')) ? fs.readdirSync(p.join(OUT, 'reference')).map(f => 'reference/' + f) : [];
  let html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MAHPLAZA v4 — AAA closure review</title>
...
<p class="lead">Actual-runtime captures from <code>mahworld/scene/mahplaza.html</code> on the permitted renderer. Revision <code>${esc(report.revision)}</code>, captured ${esc(report.ranAt)}, ${esc(report.chromium || '')} with software GL. BASELINE frames are the v3 slice (<code>../mahplaza-v3/</code>); AFTER frames are this pass. Modules present at capture: <code>${esc(JSON.stringify(report.modules || {}))}</code>.</p>`;
  ...
  for (const [view, title] of VIEWS) { const b = BASELINE[view], c = cap(view, 'night', 'wide'); if (!c) continue; if (b && fs.existsSync(p.join(BASE, b))) html += fig('../mahplaza-v3/' + b, 'v3 — ' + title + (baseInfo(view) ? ' — ' + baseInfo(view).drawCalls + ' draws · ' + baseInfo(view).triangles.toLocaleString() + ' tris' : ''), 'BASELINE'); html += fig(c.file, 'v4 — ' + title + ' — ' + meta(c), 'AFTER'); }
...
<p>Baseline v3 (night, wide): ${esc(baseCap.filter(c => c.label === 'wide' && c.time === 'night').map(c => c.view + ' ' + c.drawCalls + ' draws / ' + c.triangles.toLocaleString() + ' tris').join(' · '))}</p>
<h3>Changed files against 068fea2 (v3)</h3><pre>${esc(changed)}</pre>
