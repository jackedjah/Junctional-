...
required=['index.html','mygym.js','mygym.css','calendar.html','calendar.js','calendar.css','sw.js','netlify.toml','netlify/functions/mygym.js','netlify/functions/_mahfitt-role-context.js','supabase/migrations/052_mahfitt_role_context.sql','dev/mahfitt-role-simulator.html']
for rel in required:
...
if "fob-shell-v447" not in sw: errors.append('SW cache identity is not v447')
if 'const V = 447;' not in (ROOT/'netlify/functions/mygym.js').read_text(): errors.append('mygym asset stamp is not 447')
if 'v=447' not in (ROOT/'calendar.html').read_text(): errors.append('calendar asset stamp is not 447')
print(f'JS {len(js)} | HTML {len(html)} | CSS {len(css)} | JSON {len(jsons)}')
if errors:
    print('R90 RELEASE GATE FAIL')
    for e in errors: print(' - '+e)
    sys.exit(1)
print('R90 RELEASE GATE PASS')
