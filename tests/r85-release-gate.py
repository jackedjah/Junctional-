...
if "fob-shell-v449" not in sw: errors.append('SW cache identity is not v449')
if 'const V = 449;' not in (ROOT/'netlify/functions/mygym.js').read_text(): errors.append('mygym asset stamp is not 449')
if 'v=449' not in (ROOT/'calendar.html').read_text(): errors.append('calendar asset stamp is not 449')
print(f'JS {len(js)} | HTML {len(html)} | CSS {len(css)} | JSON {len(jsons)}')
if errors:
    print('R85 RELEASE GATE FAIL')
    for e in errors: print(' - '+e)
    sys.exit(1)
print('R85 RELEASE GATE PASS')
