...
export async function createMahplaza(canvas, options = {}) {
  ...
  const RAINM = await optional('./mah-rain.js');       /* R6: MAH RAIN and THE CRYSTAL SEA */
  const NEXUSM = await optional('./mah-nexus.js');     /* R6: MAH NEXUS, the ground-to-halo organism */
  ...
  /* R6 §1 — MAH NEXUS, the ground-to-halo transport organism. It stands on the rainforest pass
     axis at bearing 126, r 1750, and it is built BEFORE the rain because it is opaque and the rain
     curtain has to sort against it.

     Its site was the most expensive measurement in this round: four instrument failures in a row
     each made the world look like something it is not, the last and worst being an object filter
     that classified all five terrain meshes as sky because TERRAIN CONTAINS RAIN. The module header
     records every one of them, because the failures are more reusable than the answer. */
  if (NEXUSM && NEXUSM.buildMahNexus) {
    try {
      mahNexus = NEXUSM.buildMahNexus(ctx, {
        /* the pass floor, measured — not terrain.js's BASIN.y, which is the WATER plane. The two
           differ by 1.3 m and a base sunk 1.3 m into its own site is a defect nobody would see
           until a camera stood on it. */
        groundY: NEXUSM.NEXUS.GROUND_Y
      });
      scene.add(mahNexus.group);
    } catch (e) { console.info('MAHPLAZA: mah nexus module failed —', e && e.message); mahNexus = null; }
  }
  /* R6 — MAH RAIN falls from the DOME's perimeter, so the dome is asked where that is rather than
     the rain module holding a second copy of the number (L42, and the dome may yet move). The
     ground level it lands at is terrain.js's BASIN.y, the world's one natural water plane — the
     same number lakecity.js uses, and now the level of the largest body of water in the world.

     It is built LAST of the R5/R6 layers because it is transparent and it crosses everything: the
     curtain runs from 1841 m down to the sea, through the halo's outer rim, past the districts and
     the threshold, and a transparent object added before the solids it overlaps sorts against a
     depth buffer that is not there yet. */
  if (RAINM && RAINM.buildMahRain) {
    try {
      const domeR = (haloDome && DOMEM && DOMEM.DOME) ? DOMEM.DOME.R : undefined;
    ...
    } catch (e) { console.info('MAHPLAZA: mah rain module failed —', e && e.message); mahRain = null; }
  }
  ...
  function setWorldTheme(name) {
    ...
    [city, dressing, matchInterior, life, clouds, fobeams, fobstations, monument, fobpods, broadcast, lakeCity, rainforest, mahAscent, mahDescent, outerRing, facilities, beasts, interlink, halo, haloDistricts, haloLife, haloThreshold, mahCrown, haloDome, mahHaven, mahRain, mahNexus].forEach(mod => { if (mod && typeof mod.setTheme === 'function') { try { mod.setTheme(theme); } catch (e) {} } });
  ...
  if (halo && HALOM && HALOM.haloFloor) {
    ...
    /* R6: the crystal sea's BED, not its surface. A walker in the shallows stands on the bottom;
       lifting them to the waterline is the swim system's job, and ctx.swimVolumes is where this
       file publishes the shape that system will need. */
    if (mahRain && RAINM && RAINM.crystalSeaBed) surfaces.push(RAINM.crystalSeaBed);
    if (mahNexus && NEXUSM && NEXUSM.nexusSurface) surfaces.push(NEXUSM.nexusSurface);
  ...
  const navDest = () => {
    ...
    if (mahNexus && mahNexus.navSites) {
      try { for (const S of mahNexus.navSites()) list.push(S); } catch (e) {}
    }
    if (mahRain && mahRain.navSites) {
      try { for (const S of mahRain.navSites()) list.push(S); } catch (e) {}
    }
  ...
  function updateDetailTiers() {
    ...
    /* R6: MAH RAIN's tier is measured from the CURTAIN, not from the world origin — the curtain is
       an annulus at r 3434 and a viewer standing at the centre of the world is 3.4 km from it while
       being 0 m from the origin, which is the distance that decides whether 3200 shards are worth
       drawing. */
    if (mahRain && mahRain.setDetail) {
      const rE = Math.hypot(_detEye.x, _detEye.z);
      mahRain.setDetail(Math.hypot(Math.abs(rE - mahRain.stats.curtain.topR),
        _detEye.y - mahRain.stats.curtain.groundY));
    }
    /* R6: MAH NEXUS tiers on the distance to its OWN CENTRE, which is 1750 m off the world origin.
       Tiering it on |eye| would hide the complex whenever the viewer stood beside it and draw it at
       full detail from across the world — the same class of error the rain block above records. */
    if (mahNexus && mahNexus.setDetail && mahNexus.stats.centre) {
      const c = mahNexus.stats.centre;
      mahNexus.setDetail(Math.hypot(_detEye.x - c[0], _detEye.z - c[1]));
    }
  ...
  function stepWorld(t, dt, nowMs) {
    ...
    if (mahRain && mahRain.update) mahRain.update(dt, t);
    if (mahNexus && mahNexus.update) mahNexus.update(dt, t);
