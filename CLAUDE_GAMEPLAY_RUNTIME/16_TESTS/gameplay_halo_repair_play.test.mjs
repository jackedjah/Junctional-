...
send('ENTER_ROOM', { room: 'FIELD' }); run(0.2); place(HALO_LAYOUT.ground_dock.x, HALO_LAYOUT.ground_dock.z); run(0.1);
...
var volleyContacts = 0; for (var rally = 0; rally < 7; rally++) { var hv = send('HALO_SPORT_HIT', { style: rally % 2 ? 'DRIVE' : 'LOFT', lane: rally % 3 - 1 }); if (hv.accepted) volleyContacts++; run(3.1); var afterPoint = snap(); if (afterPoint.sport && afterPoint.sport.phase === 'POINT') run(1.0); }
var volleyResult = snap();
ok('7 actual player and bot contacts advance rally scoring to a result', volleyContacts === 7 && volleyResult.sport.phase === 'RESULT' && volleyResult.sport.score.bot === 7 && volleyResult.sport.contact_seq >= 10 && volleyResult.sport.last_contact && volleyResult.sport.last_contact.who === 'BOT', { contacts: volleyContacts, sport: volleyResult.sport });
