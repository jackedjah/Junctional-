...
ok('G01/G09 shipped client uses opaque owned road cores, one patch proxy, dense multifaceted bushes, restrained hint and map legend', /mahworld_path_core_owned/.test(terrainSrc) && !/transparent: true, opacity: 0\.85, polygonOffset/.test(terrainSrc) && /OctahedronGeometry\(1, 0\)/.test(gearSrc) && /SHARDS_PER_PATCH = 48/.test(gearSrc) && /MAHGIC CRYSTALS/.test(gearSrc) && /◇ MAHGIC crystals/.test(menuSrc));
