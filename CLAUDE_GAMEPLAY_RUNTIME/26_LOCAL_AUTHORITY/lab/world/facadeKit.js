...
export function createFacadeKit(THREE, opts) {
  ...
  function showMaterial(base, o) { o = o || {}; var m = base.clone(), cols = (opts.classColor || CLASS_COLOR).map(function (h) { return new THREE.Color(h); });
    m.onBeforeCompile = function (sh) { sh.uniforms.uShow = { value: showForce ? 1 : 0 }; sh.uniforms.uPhase = { value: 0 }; sh.uniforms.uTime = { value: 0 }; sh.uniforms.uCtr = { value: new THREE.Vector2(o.cx || 0, o.cz || 0) }; cols.forEach(function (c, i) { sh.uniforms['uC' + i] = { value: c }; }); m.userData.shader = sh;
      ...
      sh.fragmentShader = sh.fragmentShader.replace('#include <common>', '#include <common>\nuniform float uTime; uniform vec2 uCtr; varying vec3 vFkW;\n' + SHOW_GLSL).replace('#include <color_fragment>', '#include <color_fragment>\nif (uShow > 0.001) diffuseColor.rgb *= mix(vec3(1.0), fkShowColour() * 0.55, uShow);').replace('#include <emissivemap_fragment>', '#include <emissivemap_fragment>\nif (uShow > 0.001) { float ang = atan(vFkW.z - uCtr.y, vFkW.x - uCtr.x) / 6.2832; float wave = 0.5 + 0.5 * sin(6.2832 * (uTime * 0.3 - ang * 3.0)); totalEmissiveRadiance = mix(totalEmissiveRadiance, fkShowColour() * (0.7 + 0.5 * wave) * (0.2 + 0.8 * fkShowDim()), uShow * 0.92); }'); };   /* kept below ~1.2 so tone mapping does not bleach the class hue to white */
