...
export function createEntityStatus(THREE, opts) {
  ...
  function draw(st) { var c = st.canvas, g = st.ctx; g.clearRect(0, 0, PX, PY); var ko = !!st.ko;
    ...
    g.fillStyle = st.tint || '#dfe6ee'; g.beginPath(); g.moveTo(22, 20); g.lineTo(30, 30); g.lineTo(22, 40); g.lineTo(14, 30); g.closePath(); g.fill();   /* the square-diamond mark in the species / class tint */
    ...
    var f = st.max > 0 ? Math.max(0, Math.min(1, st.hp / st.max)) : 1; if (f > 0 && !ko) { g.fillStyle = f < 0.3 ? '#ff5a6a' : (st.hostile ? '#6e9dff' : '#e6ecf6'); rr(g, tx0 + 1, ty + 1, Math.max(2, (tx1 - tx0 - 2) * f), th - 2, 3); g.fill(); }
