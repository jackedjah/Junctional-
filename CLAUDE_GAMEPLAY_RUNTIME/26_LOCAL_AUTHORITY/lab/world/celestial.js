...
export function createCelestial(ctx) {
  ...
  function cloudTex(size, nightCloud) {
    ...
    for (var i = 0; i < 14; i++) { var x = c.width * (0.05 + i / 13 * 0.9 + (rnd() - 0.5) * 0.025), y = c.height * (0.57 - (i % 3 === 1 ? 0.14 : 0) + (rnd() - 0.5) * 0.16), rx = c.width * (0.075 + rnd() * 0.055), ry = c.height * (0.22 + rnd() * 0.16); g.save(); g.translate(x, y); g.scale(1, ry / rx); var gr = g.createRadialGradient(-rx * 0.12, -rx * 0.18, rx * 0.04, 0, 0, rx); gr.addColorStop(0, nightCloud ? 'rgba(82,101,137,0.92)' : 'rgba(146,165,185,0.9)'); gr.addColorStop(0.55, nightCloud ? 'rgba(58,76,110,0.8)' : 'rgba(126,145,166,0.76)'); gr.addColorStop(0.82, nightCloud ? 'rgba(157,188,226,0.42)' : 'rgba(235,244,252,0.44)'); gr.addColorStop(1, 'rgba(42,62,94,0)'); g.fillStyle = gr; g.fillRect(-rx, -rx, rx * 2, rx * 2); g.restore(); }
  ...
  function attachSkyEffects(body, cfg) {
    ...
    var veilD = apparentDiameter(body.dist, cfg.halo_deg * 1.35); var veil = plane(body.name + '_CLOUD_VEIL', veilD, cfg.cloudTex, 0xffffff, 0, SKY_RENDER_ORDER + 3, THREE.NormalBlending, 0xc10d5eed);
  ...
  function applyOptics(body, state) {
    ...
    for (var i = 0; i < body.materials.length; i++) { var m = body.materials[i], base = materialRecord(m); if (m.color && base.color) m.color.copy(base.color).multiplyScalar(0.48 + 0.52 * disc); if (m.emissiveIntensity !== undefined) m.emissiveIntensity = base.emissiveIntensity * (0.32 + 0.68 * disc); }
  ...
  function applyVisibility() { for (var i = 0; i < bodies.length; i++) { var b = bodies[i]; var v = b.dayVisible ? !night : night; b.root.visible = v; for (var k = 0; k < b.effects.length; k++) b.effects[k].visible = v; if (v) applyOptics(b, cloudStates[b.kind]); } }
  ...
  function build() {
    ...
    return Promise.all([sunJob, moonJob]).then(function (loaded) {
      ...
      if (S) {
        var sd = celestialDirection(S, 'sun'), sunMap = ownTexture(prepTexture(loaded[0] && loaded[0].maps && loaded[0].maps.map, true)); var sunMat = new THREE.MeshBasicMaterial({ map: sunMap || null, color: 0xfff0c8, toneMapped: false, fog: false, vertexColors: true });
        addProceduralBody({ kind: 'sun', name: 'JOBB_SUN', dir: sd, dist: +S.distance_m || 900, apparent_deg: +S.apparent_deg || 6.5, material: sunMat, seed: 7, dayVisible: true, provenance: sunInfo ? { mode: 'DERIVATIVE_TEXTURE_PROCEDURAL_GEOMETRY', derivative_id: sunInfo.derivative.id, lod: sunInfo.lod, runtime: sunInfo.file.runtime } : null, effects: { halo_deg: +S.halo_deg || 14, haloTex: radialTex(256, [[0, 'rgba(255,246,222,0.9)'], [0.22, 'rgba(255,240,206,0.62)'], [0.5, 'rgba(255,226,180,0.2)'], [1, 'rgba(255,214,160,0)']]), haloTint: 0xfff1d2, haloOpacity: 0.5, faintScale: 1.9, faintOpacity: 0.13, rayTex: rayTex(256, ['rgba(255,246,218,0.34)', 'rgba(255,225,170,0.1)', 'rgba(255,220,160,0)']), rayTint: 0xffedca, rayOpacity: 0.15, cloudTex: cloudTex(512, false), cloudOpacity: 0.88 } });
      }
      var md = celestialDirection(Mn, 'moon');
      addDerivativeBody({ kind: 'moon', name: 'JOBB_MOON', dir: md, dist: +Mn.distance_m || 900, apparent_deg: +Mn.apparent_deg || 6.2, dayVisible: false, effects: { halo_deg: +Mn.halo_deg || 12, haloTex: radialTex(256, [[0, 'rgba(190,210,255,0.7)'], [0.3, 'rgba(150,180,255,0.36)'], [0.55, 'rgba(120,160,255,0.14)'], [1, 'rgba(70,100,210,0)']]), haloTint: 0xdbe6ff, haloOpacity: 0.5, faintScale: 1.8, faintOpacity: 0.14, rayTex: rayTex(256, ['rgba(184,205,255,0.3)', 'rgba(113,145,230,0.1)', 'rgba(80,110,210,0)']), rayTint: 0xaec8ff, rayOpacity: 0.13, cloudTex: cloudTex(512, true), cloudOpacity: 0.94 } }, loaded[1], moonInfo);
