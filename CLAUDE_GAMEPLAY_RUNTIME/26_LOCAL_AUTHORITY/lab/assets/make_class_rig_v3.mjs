...
import { buildLegs, makeUvLookup } from './rig_legs.mjs';
...
['L', 'R'].forEach(function (S) {
  ...
  L['UPPERARM_' + S] = [upX * 0.82, shY, upZ];                      /* inside the humeral head, not on the deltoid surface */
...
var HIPY = CROTCH, KNEEY = (MEAS.legs && MEAS.legs.knee_h) || (CROTCH - 0.10), TIPYL = Math.max(0.02, ((MEAS.legs && MEAS.legs.tip_h) || 0) + 0.03);
...
function legWeights(y) {   /* blend bands are fractions of the MEASURED thigh and shin lengths, so the knee bends at the knee */
  /* the blend zone is the joint's own diameter: a knee 3.4 cm across blends over ±3.4 cm (in H units), so the inside of a
     bend compresses over a whole joint instead of a single ring — this is what the leg-surface test measures as strain */
  var kneeDiaH = 2 * ((MEAS.legs && MEAS.legs.knee_r) ? MEAS.legs.knee_r * 0.62 : 0.03) / H;
  /* ...but never more than 40 % of the measured thigh: Titan / Bage have a 9 cm thigh, and a 7 cm band would hand the hip to the shin */
  var kbUp = Math.max(0.015, Math.min(Math.max(0.03, kneeDiaH), 0.4 * (HIPY - KNEEY)));        /* above the knee: bounded by the thigh */
  var kbDown = Math.max(0.03, kneeDiaH * 1.15);                                                    /* below the knee: the shin is long, so the blend can be */
  var h = (y - minY) / H, tipC = TIPYL + 0.30 * (KNEEY - TIPYL), tb = Math.max(0.03, 0.4 * (KNEEY - TIPYL));
  var thigh = smoothstep(KNEEY - kbDown, KNEEY + kbUp, h), tip = 1 - smoothstep(tipC - tb, tipC + tb, h), shin = Math.max(0, 1 - thigh - tip);
  return { thigh: thigh, shin: shin, tip: tip };
}
var lowerPts = hipBand.map(function (i) { return { x: vx(i), y: P[i * 3 + 1], z: vz(i), h: vy(i) }; });
var uvOf = makeUvLookup(P, UV, hipBand, 0.02 * (H / 0.9865));
var LEGS = buildLegs(lowerPts, { minY: minY, H: H, narrow: NARROW, zs: ZS, cx: CX, hipH: HIPY - 0.005, kneeH: KNEEY, tipH: Math.max(0.006, TIPYL - 0.035), seg: 22 }, uvOf);
...
['L', 'R'].forEach(function (S) { var cx = (S === 'L' ? -1 : 1) * CX; J.push({ name: 'THIGH_' + S, parent: 1, p: [cx, Y(HIPY - 0.005), lowerZc] }); J.push({ name: 'SHIN_' + S, parent: J.length - 1, p: [cx, Y(KNEEY), lowerZc] }); J.push({ name: 'TIP_' + S, parent: J.length - 1, p: [cx, Y(TIPYL), lowerZc] }); });
...
var out = { asset: { version: '2.0', generator: 'MAHWORLD make_class_rig_v3.mjs (measured joints + geodesic binding; split legs; source geometry untouched)', extras: { mahworld_preview: 'DEVELOPMENT_PREVIEW_UNAPPROVED', source: path.basename(SRC), source_sha256: srcSha } }, scene: 0, scenes: [{ nodes: [0, 1, jn[0]] }], nodes: nodes,
