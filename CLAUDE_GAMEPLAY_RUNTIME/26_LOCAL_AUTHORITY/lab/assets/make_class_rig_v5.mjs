...
import { makeUvLookup } from './rig_legs.mjs'; import { buildLegsV4 } from './rig_legs_v4.mjs'; import { buildLegsSurgical } from './rig_legs_surgical.mjs'; import { buildLegsFaithful } from './rig_legs_faithful.mjs'; import { unweldAxilla } from './rig_axilla.mjs';
...
var BLEND = parseFloat(arg('--blend', '0.85'));
var V5 = { column_min_arm: parseFloat(arg('--col-min-arm', '0.45')), column_medial_t: parseFloat(arg('--col-medial-t', '0.3')), column_medial_margin: 0.005, column_medial_rad: parseFloat(arg('--col-medial-rad', '1.0')), gh_inward_from_threshold: parseFloat(arg('--gh-in', '0.011')), gh_below_top: parseFloat(arg('--gh-down', '0.037')), hip_h: parseFloat(arg('--hip-h', '0.4575')), hip_x: parseFloat(arg('--hip-x', '0.0568')), hip_z: parseFloat(arg('--hip-z', '0.015')), sc_x: parseFloat(arg('--sc-x', '0.012')), sc_h: parseFloat(arg('--sc-h', '0.8109')), sc_z: parseFloat(arg('--sc-z', '0.027')), scap_x: parseFloat(arg('--scap-x', '0.0618')), scap_z: parseFloat(arg('--scap-z', '-0.076')) };   /* forensic numbers (source units / H fractions) */   /* sigma multiplier on the measured limb thickness */
...
var HIPY = CROTCH, KNEEY = ((MEAS.legs && MEAS.legs.knee_h) || (CROTCH - 0.10)) + 0.010, HIP_PIVOT = V5.hip_h, DOME_TOP = HIP_PIVOT - 0.006, TIPYL = Math.max(0.02, ((MEAS.legs && MEAS.legs.tip_h) || 0) + 0.03);
...
var TIPS = { PELVIS: J[BI.LUMBAR].p, LUMBAR: J[2].p, SPINE: J[BI.SPINE2].p, SPINE2: J[3].p, CLAV_L: J[7].p, CLAV_R: J[11].p, SCAP_L: J[7].p, SCAP_R: J[11].p, ARMTWIST_L: J[8].p, ARMTWIST_R: J[12].p, FORETWIST_L: J[9].p, FORETWIST_R: J[13].p, HEAD: [0, Y(1.0), 0], HAND_L: [L.HANDC_L[0], Y(L.HANDC_L[1]), L.HANDC_L[2]], HAND_R: [L.HANDC_R[0], Y(L.HANDC_R[1]), L.HANDC_R[2]], TAIL3: [0, minY, 0], FINGERS_L: [L.FINGERS_L[0], Y(L.TIPY_L), L.FINGERS_L[2]], FINGERS_R: [L.FINGERS_R[0], Y(L.TIPY_R), L.FINGERS_R[2]] };
...
var GEO_AXIAL = ['PELVIS', 'LUMBAR', 'SPINE', 'SPINE2', 'CHEST', 'NECK', 'HEAD', 'TAIL1', 'TAIL2', 'TAIL3'];
...
(function () {
  ...
  ['L', 'R'].forEach(function (S) { var gh = J[BI['UPPERARM_' + S]].p, el = J[BI['FOREARM_' + S]].p; var axis = [el[0] - gh[0], el[1] - gh[1], el[2] - gh[2]]; var Lax = Math.hypot(axis[0], axis[1], axis[2]); axis = axis.map(function (c) { return c / Lax; }); var rArm = 0.052;
    for (var i2 = 0; i2 < N; i2++) { if ((vx(i2) < 0 ? 'L' : 'R') !== S) continue; var w2 = W[i2]; var d = [vx(i2) - gh[0], P[i2 * 3 + 1] - gh[1], vz(i2) - gh[2]]; var t = (d[0] * axis[0] + d[1] * axis[1] + d[2] * axis[2]) / Lax; if (t < 0.04 || t > 1.05) continue; var along = t * Lax; var rad = Math.sqrt(Math.max(0, d[0] * d[0] + d[1] * d[1] + d[2] * d[2] - along * along)); if (rad > 1.35 * rArm) continue;
      ...
      /* S10 AXILLARY-PLATE GUARD: in the hanging rest pose the humeral axis passes within ~13 cm of the ribcage side, so this rule used to seize
         lat / ribcage verts (originally CHEST-SPINE2-LUMBAR) 13-22 cm below the joint and hand them 95 % to the arm next to torso-bound neighbours —
         1.8 cm edges became 55 cm plates at 150° (deploy/_stretch.mjs). Only verts that are already mostly arm are boosted, and nothing medial
         of the joint below the head is touched. */ if (armSum < V5.column_min_arm) continue; if (t > V5.column_medial_t && Math.abs(vx(i2)) < Math.abs(gh[0]) + V5.column_medial_margin && rad > V5.column_medial_rad * rArm) continue;   /* the inner arm itself (rad ≤ 1.0 r) is still arm; only the ribcage shell beyond it is left to the trunk */
...
var NARROW = 0.5, ZS = 0.82, CX = V5.hip_x;   /* v5: leg columns centred on the measured femoral heads (0.27 of the 0.2104 pelvis width) */
...
function legWeights(y) {   /* v4: the knee blends over ±0.02 H around the measured knee (a joint's own diameter); the thigh owns everything up to the dome */
  var h = (y - minY) / H, kb = LEGS_MODE === 'faithful' ? KNEE_BLEND : 0.020, tipC = TIPYL + 0.30 * (KNEEY - TIPYL), tb = Math.max(0.03, 0.4 * (KNEEY - TIPYL));
...
var LEGS_MODE = arg('--legs', 'surgical'); var KNEE_BLEND = parseFloat(arg('--knee-blend', '0.06'));   /* faithful legs: the source tail's own triangles are 2–3 cm long, so the knee blends over ±0.06 H (measured 2026-09-18: ±0.04 left 26 inverted triangles in the test swing, ±0.055 16 in the gather, ±0.06–0.07 none; a dense loft could use ±0.02) */ var shellHW = Math.max.apply(null, slice(CROTCH - 0.006, CROTCH + 0.006, function (i) { return Math.abs(vx(i)) < 0.125; }).map(function (i) { return Math.abs(vx(i)); }).concat([0]));   /* pelvis shell half-width at the crotch line: the split legs must stay inside it */
var LEGS = LEGS_MODE === 'faithful' ? buildLegsFaithful({ P: P, NM: NM, UV: UV, IDX: IDX, lowerTris: lowerTris }, { minY: minY, H: H, Y: Y, splitH: HIPY, kneeH: KNEEY, domeTop: DOME_TOP, inner: parseFloat(arg('--leg-inner', '0.31')), ramp_h: parseFloat(arg('--leg-ramp', '0.08')), shellHW: shellHW, kneeBlend: KNEE_BLEND })   /* CONVERGENCE O6 (PASS 4B): the fused lower body's own triangles, cut at the sagittal plane; medial walls inferred (rig_legs_faithful.mjs) */
  : LEGS_MODE === 'v4' ? buildLegsV4(axialPts, { minY: minY, H: H, narrow: NARROW, zs: ZS, cx: CX, domeTop: DOME_TOP, splitH: HIPY, kneeH: KNEEY, tipH: Math.max(0.006, TIPYL - 0.035), seg: 24, sigma: 0.010 }, uvOf)
  : buildLegsSurgical(axialPts, { minY: minY, H: H, narrow: NARROW, zs: ZS, cx: CX, bulge: parseFloat(arg('--leg-bulge', '0.62')), domeTop: DOME_TOP, splitH: HIPY, kneeH: KNEEY, tipH: Math.max(0.006, TIPYL - 0.035), seg: 24, sigma: 0.010, shellHW: shellHW, zcTip: lowerZc }, uvOf);   /* S13: legs derived from the fused anatomy (rig_legs_surgical.mjs) */
if (LEGS.stats) console.log(LEGS_MODE + ' legs', JSON.stringify(LEGS.stats), 'shell hw', shellHW.toFixed(4));
...
['L', 'R'].forEach(function (S) { var cx = (S === 'L' ? -1 : 1) * CX; J.push({ name: 'THIGH_' + S, parent: 1, p: [cx, Y(HIP_PIVOT), V5.hip_z] });   /* v5: femoral head at the measured pelvis mass: mid-height of the widest band, just forward of its mid-depth */ J.push({ name: 'SHIN_' + S, parent: J.length - 1, p: [cx, Y(KNEEY), lowerZc] }); J.push({ name: 'TIP_' + S, parent: J.length - 1, p: [cx, Y(TIPYL), lowerZc] }); });
...
/* v5 SHOULDER CORRECTIVE LIBRARY — one targeted corrective per elevation band per side, blended by elevation windows at runtime
...
/* ---------------- S10: AXILLA UNWELD (runtime derivative; see rig_axilla.mjs) ---------------- */
var AXILLA = null;
if (!flag('--no-axilla')) {
  var MG = Array.from(MGROUP);
  var allUpper = (function () { var m = {}; for (var q = 0; q < upperIdx.length; q++) m[upperIdx[q]] = 1; return Object.keys(m).map(Number); })();
  var uvAll = makeUvLookup(P, UV, allUpper, 0.02 * ((maxY - minY) / 0.9865));
  var pushVertex = function (pos, nrm, uv, srcVert) { var id = bodyPos.length / 3; bodyPos.push(pos[0], pos[1], pos[2]); bodyNorm.push(nrm[0], nrm[1], nrm[2]); bodyUV.push(uv[0], uv[1]); for (var k = 0; k < 4; k++) { bodyJa.push(bodyJ[srcVert * 4 + k]); bodyWa.push(bodyW[srcVert * 4 + k]); } MG.push(0); return id; };
  var pushTri = function (i0, i1, i2) { var ax = bodyPos[i1 * 3] - bodyPos[i0 * 3], ay = bodyPos[i1 * 3 + 1] - bodyPos[i0 * 3 + 1], az = bodyPos[i1 * 3 + 2] - bodyPos[i0 * 3 + 2]; var bx = bodyPos[i2 * 3] - bodyPos[i0 * 3], by = bodyPos[i2 * 3 + 1] - bodyPos[i0 * 3 + 1], bz = bodyPos[i2 * 3 + 2] - bodyPos[i0 * 3 + 2]; var nx = ay * bz - az * by, ny = az * bx - ax * bz, nz = ax * by - ay * bx; var d = nx * bodyNorm[i0 * 3] + ny * bodyNorm[i0 * 3 + 1] + nz * bodyNorm[i0 * 3 + 2]; if (d < 0) upperIdx.push(i0, i2, i1); else upperIdx.push(i0, i1, i2); };
  AXILLA = unweldAxilla({ P: P, N: N, J: J, BI: BI, bodyJ: bodyJ, bodyW: bodyW, upperIdx: upperIdx, rArm: 0.052, uvOf: uvAll, UV: UV, NM: NM, jointName: function (ji) { return J[ji].name; }, torsoHalf: torsoHalf, pushVertex: pushVertex, pushTri: pushTri });
  MGROUP = new Float32Array(MG); console.log('axilla unweld:', JSON.stringify(AXILLA));
}
/* ---------------- write GLB ---------------- */
...
var out = { asset: { version: '2.0', generator: 'MAHWORLD make_class_rig_v5.mjs (anatomical shoulder complex + twist + distributed trunk + femoral-head hips + corrective library; source geometry untouched)', extras: { mahworld_preview: 'DEVELOPMENT_PREVIEW_UNAPPROVED', source: path.basename(SRC), source_sha256: srcSha } }, scene: 0, scenes: [{ nodes: [0, 1, jn[0]] }], nodes: nodes,
...
/* ---------------- manifest ---------------- */
var mf = { export_id: ID, version: VERSION, sha256: outSha, owner_review_status: 'CANDIDATE', rig_generator: 'make_class_rig_v5.mjs', rig_chain: 'PELVIS>LUMBAR>SPINE>SPINE2>CHEST>NECK>HEAD; CLAV>SCAP>UPPERARM>ARMTWIST; FOREARM>FORETWIST', glenohumeral: { L: L.UPPERARM_L, R: L.UPPERARM_R, cap_apex_L: L.CAP_APEX_L, head_radius_L: L.HEAD_R_L }, v5: V5, hip_pivot_h: HIP_PIVOT, knee_h: KNEEY, muscles: MUSCLES.map(function (m) { return { name: m.name, group: m.group, verts: m.verts.length }; }), muscle_attribute: '_MUSCLE', preview_status: 'DEVELOPMENT_PREVIEW_UNAPPROVED (measured derivative auto-rig v4: v3 + cross-section joints + geodesic surface binding, split legs, face and finger bones over the Tripo source; not a retained or production character)', source: { file: path.basename(SRC), sha256: srcSha }, source_release: 'isolated derivative of ' + path.basename(SRC) + ' (sha256 ' + srcSha + ')', units: 'metres', up_axis: '+Y', forward_axis: '-Z', source_forward_axis: '+Z', root: 'ROOT', scale_to_metres: +(1.8 / H).toFixed(5), source_height_units: +H.toFixed(4), forms: ['FUSED', 'SPLIT'], clips: {},
  sockets: { ROOT: 'ROOT', HEAD: 'HEAD', CHEST: 'CHEST', HAND_L: 'HAND_L', HAND_R: 'HAND_R', FEET: 'TAIL3', TIP_FUSED: 'TAIL3', TIP_L: 'TIP_L', TIP_R: 'TIP_R' },
  split: { legs_mesh: ID + '_SPLIT_LEGS', packed_morph: 'PACKED', fused_lower_role: 'fused_lower', upper_role: 'upper', leg_bones: ['THIGH_L', 'SHIN_L', 'TIP_L', 'THIGH_R', 'SHIN_R', 'TIP_R'], leg_centres_x: [-CX, CX], crotch_h: CROTCH, method: LEGS_MODE === 'faithful' ? ('FAITHFUL (CONVERGENCE O6, PASS 4B): each leg’s LATERAL wall is the fused lower body’s OWN triangles cut at the sagittal plane (source vertices, normals and UVs kept; clipped triangles get seam vertices on their own edges — every triangle stays inside its texture island); the MEDIAL wall is that wall mirrored and compressed toward the seam plane (x’ = −' + LEGS.stats.inner + '·x, inverse-transpose normals, mirrored UVs) and welded along the seam; a shallow cone closes the top inside the pelvis; the separated form shifts each leg laterally by ' + LEGS.stats.inner + ' × the thigh half-width so the medial walls meet at the midline; PACKED = the exact source positions for the lateral wall and the medial wall folded onto the sagittal plane (the packed pair IS the fused silhouette) · ' + JSON.stringify(LEGS.stats.sides)) : (LEGS_MODE === 'v4' ? 'each leg = a closed loft of ' + LEGS.rings + ' elliptical rings x ' + LEGS.seg + ' segments sized from the fused body sections at every height (x ' + NARROW + ', z ' + ZS + ')' : 'SURGICAL (S13): each leg = a closed loft of ' + LEGS.rings + ' rings x ' + LEGS.seg + ' segments whose OUTER half is the fused body’s own cross-section outline at that height (angular sampling, smoothed along the limb; narrowed to ' + NARROW + ' of the fused half-width below the knee, flush with the pelvis shell at the crotch) and whose INNER half is that outline mirrored and compressed into a D-section (bulge ' + arg('--leg-bulge', '0.62') + ') — inferred from the species anatomy, not a generic cylinder') + ', dense loops around the measured knee with a patella swell, closed tip point and hip dome, UVs from the nearest fused-body vertex; PACKED delta presses the same topology onto its half of the fused silhouette (no seam plane, manifold)' },
  ...
  limitations: ['derivative auto-rig: joint placement measured from cross-sections, smooth distance weights (not hand-painted); shoulder / elbow / groin can pinch at extreme angles', (LEGS_MODE === 'faithful' ? 'split legs (CONVERGENCE faithful): lateral walls = the fused lower body’s own triangles; medial walls inferred (mirror, compressed D) — the mirrored texture meets its own reflection along the front / back seam of each leg (a faint vertical line, inherent to the construction); the lateral shift ramps in below the crotch so the leg’s top ring stays inside the pelvis (above the ramp the two medial walls overlap inside the pelvis); the groin can pinch at extreme hip angles like every auto-rig here; the top cone is hidden inside the pelvis' : LEGS_MODE === 'v4' ? 'split legs are lofted ellipses sized from the fused sections (v4)' : 'split legs (S13 surgical): outer surface = the fused taper’s own outline narrowed to fit two limbs side by side, inner surface inferred as a D-section from that outline (texture mirrors on the inner face); above the split line the limb continues inside the pelvis to a dome under the femur head; the pelvis underside is capped flat'), 'PACKED morph carries positions only (normals are the separated-form normals)', 'face bones deform a Tripo head with no eyelid, iris or mouth-interior geometry — expressions are crude and stated as such', 'finger mass is one bone per hand (curl only)', 'textures resampled in the browser canvas, not mip-aware'], generated: new Date().toISOString() };
