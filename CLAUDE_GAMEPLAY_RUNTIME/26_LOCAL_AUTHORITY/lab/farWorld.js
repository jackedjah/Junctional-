...
export function createFarWorld(THREE, opts) {
  ...
  function pushGeo(acc, geo, matrix, color, facet, hazeTop) { geo = geo.toNonIndexed(); geo.applyMatrix4(matrix); geo.computeVertexNormals(); var p = geo.attributes.position, n = geo.attributes.normal; var tmpC = hazeTop ? new THREE.Color() : null; for (var i = 0; i < p.count; i++) { var tone = 1; if (facet) { var lam = n.getX(i) * sunDir.x + n.getY(i) * sunDir.y + n.getZ(i) * sunDir.z; tone = 0.8 + 0.18 * Math.max(0, lam) + (n.getY(i) > 0.8 ? 0.04 : 0); }   /* never brighter than the horizon sky: a far massif is a SILHOUETTE in blue-grey */ var c = color; if (hazeTop) { var hy = Math.max(0, Math.min(1, p.getY(i) / hazeTop)); c = tmpC.copy(color).lerp(haze, 0.42 * (1 - hy)); }   /* aerial perspective by height: the base of a massif sits in thicker air than its peak */ acc.pos.push(p.getX(i), p.getY(i), p.getZ(i)); acc.col.push(c.r * tone, c.g * tone, c.b * tone); if (acc.nrm) acc.nrm.push(n.getX(i), n.getY(i), n.getZ(i)); } }
  ...
  var mid = { pos: [], col: [], nrm: [] }; var sectors = [Math.PI * 0.62, Math.PI * 0.85, Math.PI * 1.05, Math.PI * 1.28, Math.PI * 1.5, Math.PI * 1.72];   /* bearings (atan2 x,z): the north half holds the temple / market / gym / tower — mid forms stay south, east and west */
  var midPlat = new THREE.Color(NIGHT ? 0x55657e : 0x9fb0c2), midCrystal = new THREE.Color(NIGHT ? 0x5d7aa6 : 0xb4cde6);   /* B7 §10 F: the mid forms a step darker than the far ring — three readable depth steps */
  sectors.forEach(function (b, i) { var dist = 230 + rnd() * 100; var cx = Math.sin(b) * dist, cz = Math.cos(b) * dist;
    ...
    if (i % 2 === 0) { /* one continuous distant arch with two grounded piers; the old chain of boxes read as a broken half-bridge */ var b2 = sectors[(i + 1) % sectors.length]; var d2 = 230 + rnd() * 100; var ex = Math.sin(b2) * d2, ez = Math.cos(b2) * d2; var curve = new THREE.QuadraticBezierCurve3(new THREE.Vector3(cx, 22, cz), new THREE.Vector3((cx + ex) * 0.5, 50, (cz + ez) * 0.5), new THREE.Vector3(ex, 22, ez)); pushGeo(mid, new THREE.TubeGeometry(curve, 28, 2.5, 7, false), new THREE.Matrix4(), midPlat.clone().multiplyScalar(0.9), true); [0.18, 0.82].forEach(function (u) { var pp = curve.getPoint(u), ph = Math.max(8, pp.y); pushGeo(mid, new THREE.CylinderGeometry(1.7, 3.0, ph, 8), place(pp.x, ph * 0.5, pp.z, 0, 1, 1, 1), midPlat.clone().multiplyScalar(0.82), true); }); }
  });
  ...
  var ring = new THREE.RingGeometry(250, 890, 48, 1); ring.rotateX(-Math.PI / 2); var rc = []; var rp = ring.attributes.position; var inner = new THREE.Color(NIGHT ? 0x141a24 : 0x2a3341), outer = haze.clone().lerp(new THREE.Color(NIGHT ? 0x1e2c42 : 0xb9c8d6), 0.5);
  for (var v = 0; v < rp.count; v++) { var rr = Math.hypot(rp.getX(v), rp.getZ(v)); var t = Math.max(0, Math.min(1, (rr - 300) / 590)); var c = inner.clone().lerp(outer, Math.pow(t, 0.6)); rc.push(c.r, c.g, c.b); }
  ring.setAttribute('color', new THREE.Float32BufferAttribute(rc, 3)); var hazeMesh = new THREE.Mesh(ring, new THREE.MeshBasicMaterial({ vertexColors: true, fog: false, toneMapped: false })); hazeMesh.position.y = -0.02;   /* just under the district floor (y −0.005) so the floor edge at 300 m shows no seam */ hazeMesh.name = 'HAZE_PLAIN'; hazeMesh.renderOrder = -6; hazeMesh.frustumCulled = false; group.add(hazeMesh);
