const MANIFEST_ROOT = "../../10_MANIFEST/FINAL_POLISH/";
...
export class MahfittCompanion {
  ...
  async setCharacter(character) {
    ...
    this.manifestUrl = new URL(`${MANIFEST_ROOT}${character}.json`, import.meta.url);
    this.manifest = await fetch(this.manifestUrl).then(r => {
      if (!r.ok) throw new Error(`Manifest ${r.status}`);
      return r.json();
    });
    if (this.secondaryPalette) this.manifest.themeArchitecture.palettes.app = this.secondaryPalette;
  ...
  dispose() { this.disposed = true; clearTimeout(this.timer); this.pageCache.clear(); }
  ...
  startPassiveBehavior() {
    ...
    const tick = async () => {
      ...
      if (!blocked && Math.random() < 0.18) {
        const choices = ["look_left", "look_right", "idle_long"];
