...
export function cutMrsBicepsProximalStation141(g,side){
 ...
 const protectedTriangles=new Set(['mrsBrachialisPatch127','mrsDeltoid128','mrsDeltoidInsertion129','mrsBiceps139'].flatMap(k=>g.userData[k].planes.flatMap(p=>p.triangleIndices)));
 const protectedKeys=new Set([...protectedTriangles].flatMap(t=>stock.slice(t*3,t*3+3).map(key)));
 const active=v=>{const q=xyz(v);return q.z>.025&&q.t>.24+1e-7&&q.t<.37-1e-7&&Math.abs(q.x)<.052;};
 const intersecting=vs=>{const q=vs.map(xyz);return q.every(p=>p.z>.025)&&Math.min(...q.map(p=>p.t))<.37&&Math.max(...q.map(p=>p.t))>.24&&Math.min(...q.map(p=>p.x))<.075&&Math.max(...q.map(p=>p.x))>-.095;};
 const cut=fn=>{
  const edges=new Map();
  for(const f of faces){const vs=f.ids.map(i=>nodes[i].p);if(!intersecting(vs))continue;
   for(let k=0;k<3;k++){
    ...
    const v=pa.clone().lerp(pb,t),q=xyz(v);if(q.z<=.025||q.t<.24-1e-7||q.t>.37+1e-7||(q.x<-.095||q.x>.075))continue;
  ...
  for(const f of faces){const v=f.ids,m=v.map((a,k)=>edges.get([a,v[(k+1)%3]].sort((a,b)=>a-b).join(':'))),count=m.filter(i=>i!==undefined).length;
   ...
   if(protectedTriangles.has(f.parent))throw new Error('M141 boundary would split retained crown '+side+' '+f.parent);
 ...
 g.userData.mrsBicepsBoundary141={version:'M141',oldToNewTriangles:mapping,originalTriangleCount:original.length,triangleCount:final.length,refinedBaselinePositions:expanded.flatMap(v=>v.toArray()),stations:[.30],microWelds};
}
