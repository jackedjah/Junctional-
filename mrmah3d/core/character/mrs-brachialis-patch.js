...
// Reused retained M125 bounded-field construction in an orthonormal
// brachialis chart. Fixed outer skin, measured crown, continuous return collar.
export function rebuildMrsBrachialisPatch(g,{patchSpec,characterSide}){
 ...
 for(const sign of [1]){
  ...
  for(const t of [-.8,-.2,.2,.8]){cut((u,v)=>u-t);cut((u,v)=>v-t);}
  ...
  const ring=[boundary[0].a];while(next.get(ring.at(-1))!==ring[0]){const n=next.get(ring.at(-1));if(n===undefined||ring.includes(n))throw new Error('M127 broken chart boundary');ring.push(n);}
  if(ring.length!==boundary.length)throw new Error('M127 multiple chart boundary');
  const boundaryDeviation=Math.max(...ring.map(i=>{const q=uv(nodes[i].p);return Math.abs(Math.abs(q[0])+Math.abs(q[1])-1);}));if(boundaryDeviation>1e-5)throw new Error('M127 perimeter drift '+JSON.stringify({boundaryDeviation,worst:ring.map(id=>({id,uv:uv(nodes[id].p),p:nodes[id].p.toArray()})).sort((a,b)=>(Math.abs(b.uv[0])+Math.abs(b.uv[1]))-(Math.abs(a.uv[0])+Math.abs(a.uv[1]))).slice(-4)}));
  const local=[],add=(ids,owner)=>{if(new Set(ids).size<3)return;const[a,b,c]=ids.map(i=>nodes[i].p);if((b.x-a.x)*(c.y-a.y)-(b.y-a.y)*(c.x-a.x)<0)ids=[ids[0],ids[2],ids[1]];const f={ids,old:-1,owner,side};local.push(f);added.push(f);};
  const prefix='MRS_BRACHIALIS_'+side+'_127_',grid=[],r=spec.radius,ru=spec.radiusU??r;
  for(const t of spec.stations)for(const s of spec.stations){const u=ru*(s+t)/2,v=r*(s-t)/2;grid.push(make(u,v,sample(u,v).z));}
  // One dominant facing owns the central cell. Fit only its four vertices;
  // surrounding support cells follow a coherent convex muscle envelope.
  ...
  const fit=mat.map(v=>v[3]);fit[0]=Math.max(fit[0],sample(0,0).z);
  // The old rail contains non-anatomical height noise. A single convex
  // support envelope owns the new belly; do not reproduce each old dent.
  for(const id of grid){const[u,v]=uv(nodes[id].p);nodes[id].p.z=fit[0]+fit[1]*u+fit[2]*v-.012*(spec.w/.057)**2*u*u-.008*v*v;}
  for(const id of crownIds){const[u,v]=uv(nodes[id].p);nodes[id].p.z=fit[0]+fit[1]*u+fit[2]*v;}
  ...
  // the convex belly and create a dark attachment slash. The actual perimeter
  ...
  const station=(u,v)=>{u*=ru;v*=r;const key=[u,v].map(v=>v.toFixed(7)).join(',');if(stationMap.has(key))return stationMap.get(key);
   // Coalesce nearly coincident internal chart stations. Their fixed outer
   // vertices remain separate; the collar forms a fan instead of float32
   // slivers whose computed normals cannot represent the common plane.
   for(const id of [...grid,...stationMap.values()]){const q=uv(nodes[id].p);if(Math.hypot(q[0]-u,q[1]-v)<1e-4){stationMap.set(key,id);return id;}}
   ...
   }throw new Error('M127 perimeter station lacks plane owner');
  };
  ...
  const covered=new Set(sectors.flatMap(s=>s.edgeIds.slice(1).map((id,j)=>[id,s.edgeIds[j]].sort((a,b)=>a-b).join(':'))));
  for(let i=0;i<ring.length;i++){const a=ring[i],b=ring[(i+1)%ring.length];if(!covered.has([a,b].sort((a,b)=>a-b).join(':')))throw new Error('M127 boundary chart is not monotone '+JSON.stringify({edge:[a,b],uv:[uv(nodes[a].p),uv(nodes[b].p)],near:sectors.filter(s=>s.edgeIds.includes(a)).map(s=>s.edgeIds.slice(Math.max(0,s.edgeIds.indexOf(a)-2),s.edgeIds.indexOf(a)+4).map(id=>({id,uv:uv(nodes[id].p),p:nodes[id].p.toArray()})))}));}
  ...
  const guideMap=new Map();for(const{edgeIds,inner}of sectors){const rows=[inner];
   for(const t of [.25,.5,.75])rows.push(inner.map((id,j)=>{
    ...
    const innerNormal=perimeterNormal(a);
    // Retained arm normals have hard family transitions at almost coincident
    // chart stations. A geometric secant gives neighboring collar strips one
    // coherent outer tangent instead of propagating those optical jumps.
    let m0=-(innerNormal.x*dx+innerNormal.y*dy)/innerNormal.z,m1=delta;
  ...
  // All four return sectors share one membrane solve. Independent radial
  // Hermite strips produced a transverse slope break at the side tips.
  // Hold the physical plane boundary and original outer skin exactly fixed.
  const free=[...new Set(guideMap.values())],freeSet=new Set(free),rows=new Map(free.map(id=>[id,new Map()]));
  for(const f of local){
   if(!f.owner.endsWith('RETURN'))continue;
   const p=f.ids.map(id=>nodes[id].p),a2=(p[1].x-p[0].x)*(p[2].y-p[0].y)-(p[1].y-p[0].y)*(p[2].x-p[0].x);
   const grad=p.map((_,k)=>[(p[(k+1)%3].y-p[(k+2)%3].y)/a2,(p[(k+2)%3].x-p[(k+1)%3].x)/a2]);
   for(let k=0;k<3;k++){const row=rows.get(f.ids[k]);if(!row)continue;for(let j=0;j<3;j++){const id=f.ids[j],w=Math.abs(a2)*.5*(grad[k][0]*grad[j][0]+grad[k][1]*grad[j][1]);row.set(id,(row.get(id)||0)+w);}}
  }
  const values=new Map(free.map(id=>[id,nodes[id].p.z])),res=new Map(),direction=new Map(),scaled=new Map();let rz=0;
  for(const id of free){const row=rows.get(id);let v=0;for(const[j,w]of row)v-=w*(freeSet.has(j)?values.get(j):nodes[j].p.z);res.set(id,v);const s=v/row.get(id);scaled.set(id,s);direction.set(id,s);rz+=v*s;}
  const initial=Math.sqrt(rz);let solveIterations=0;
  for(;solveIterations<3000&&Math.sqrt(rz)>Math.max(1e-12,initial*1e-9);solveIterations++){
   const ad=new Map();let den=0;for(const id of free){let v=0;for(const[j,w]of rows.get(id))if(freeSet.has(j))v+=w*direction.get(j);ad.set(id,v);den+=direction.get(id)*v;}
   if(!(den>0))throw new Error('M127 invalid coupled return solve');const alpha=rz/den;let next=0;
   for(const id of free){values.set(id,values.get(id)+alpha*direction.get(id));const r=res.get(id)-alpha*ad.get(id);res.set(id,r);const s=r/rows.get(id).get(id);scaled.set(id,s);next+=r*s;}
   const beta=next/rz;for(const id of free)direction.set(id,scaled.get(id)+beta*direction.get(id));rz=next;
  }
  if(Math.sqrt(rz)>Math.max(1e-11,initial*1e-8))throw new Error('M127 return solve did not converge');
  for(const id of free)nodes[id].p.z=values.get(id);
  ...
  if(minimumArea<5e-11||minimumForwardNormal<.12||maximumSurfaceShift>.025){const worst=local.map(f=>{const p=f.ids.map(i=>nodes[i].p),n=p[1].clone().sub(p[0]).cross(p[2].clone().sub(p[0])).normalize();return{owner:f.owner,points:p.map(v=>v.toArray()),uv:p.map(uv),normal:n.toArray()};}).sort((a,b)=>a.normal[2]-b.normal[2])[0];throw new Error('M127 invalid field '+JSON.stringify({minimumArea,minimumForwardNormal,maximumSurfaceShift,worst}));}
  reports.push({side,spec,fit,removedTriangles:patch.length,addedTriangles:local.length,boundaryVertices:ring.length,boundaryDeviation,maximumBoundaryDrift:0,maximumSurfaceShift,minimumForwardNormal,minimumArea,deltaVolume,planeCount:17,returnMethod:'single finite-element membrane solve across all collar sectors; inner and outer boundaries fixed',returnSolve:{freeNodes:free.length,iterations:solveIterations,relativeResidual:Math.sqrt(rz)/Math.max(initial,1e-30)},method:'continuous brachialis insertion field; one broad diamond crown and sixteen convex-envelope supports'});
  const patchSet=new Set(patch);work=work.filter(f=>!patchSet.has(f));
 }
 // Chart cuts can create sub-micron exterior edges next to retained vertices.
 // Collapse only those cut edges, retaining original vertices and surface;
 // do not carry zero-use sliver geometry into the authoring master.
 const parent=nodes.map((_,i)=>i),rootId=i=>{while(parent[i]!==i)i=parent[i];return i;};let microEdgeWelds=0;
 const sliverEdges=new Set();for(const f of work){if(f.old>=0)continue;const p=f.ids.map(i=>nodes[i].p),area=p[1].clone().sub(p[0]).cross(p[2].clone().sub(p[0])).length();if(area>=2e-10)continue;
  const edges=f.ids.map((a,k)=>({a,b:f.ids[(k+1)%3],length:p[k].distanceTo(p[(k+1)%3])})).sort((a,b)=>a.length-b.length);
  const e=edges.find(e=>e.length<2e-5&&(nodes[e.a].source===undefined||nodes[e.b].source===undefined));if(e)sliverEdges.add([e.a,e.b].sort((a,b)=>a-b).join(':'));
 }
 for(const f of work)for(let k=0;k<3;k++){
  let a=rootId(f.ids[k]),b=rootId(f.ids[(k+1)%3]);if(a===b)continue;
  const na=nodes[a],nb=nodes[b];if(na.source!==undefined&&nb.source!==undefined)continue;
  if(!(na.retained||na.source!==undefined)||!(nb.retained||nb.source!==undefined))continue;
  if(na.p.distanceTo(nb.p)>=1.2e-6&&!sliverEdges.has([a,b].sort((a,b)=>a-b).join(':')))continue;
  if(nb.source!==undefined||a>b&&na.source===undefined)[a,b]=[b,a];parent[b]=a;microEdgeWelds++;
 }
 const final=work.concat(added).map(f=>({...f,ids:f.ids.map(rootId)})).filter(f=>new Set(f.ids).size===3),mapping=Array(faces.length).fill(-1),children=Array.from({length:faces.length},()=>[]);final.forEach((f,i)=>{if(f.old>=0){mapping[f.old]=i;children[f.old].push(i);}else if(f.parent>=0)children[f.parent].push(i);});
 ...
 final.forEach((f,ti)=>{
  ...
  if(isPlane){if(!registry.has(f.owner))registry.set(f.owner,{id:f.owner,region:'BRACHIALIS',side:f.side,type:f.owner.endsWith('CROWN')?'elongated diamond':'directional wedge',anatomicalOwner:'brachialis belly and directional supports',triangleIndices:[],averageNormal:physical.toArray(),status:'AUTHORED; closure pending checkpoint validation',adjacentPlanes:[],blackPointAdjacency:[]});registry.get(f.owner).triangleIndices.push(ti);}
  for(let k=0;k<3;k++){
   const id=f.ids[k],node=nodes[id];for(const[name,a]of Object.entries(attr)){
    ...
    else if(f.parent>=0){const ids=faces[f.parent].ids,bary=new Triangle(...ids.map(id=>nodes[id].p)).getBarycoord(node.p,new Vector3()).toArray();values=Array.from({length:a.itemSize},(_,j)=>bary.reduce((sum,w,l)=>sum+w*a.array[(f.parent*3+l)*a.itemSize+j],0));}
    ...
    if(f.old<0){
     ...
     if(name==='aFacet'&&f.parent===undefined)values=[...physical.toArray(),values[3]];
     if(name==='aBary')values=[k===0?1:0,k===1?1:0,k===2?1:0,values[3]];
     if(f.parent===undefined&&['aSmooth','aMoldNormal','aCrystalNormal','normal'].includes(name)){
 ...
 for(const e of edgeOwners.values()){
  if(e.faces.length!==2){const beforePairs=work.concat(added).filter(f=>f.ids.includes(e.a)&&f.ids.includes(e.b)).map(f=>({ids:f.ids,owner:f.owner}));throw new Error('M127 open edge '+JSON.stringify({points:[nodes[e.a].p,nodes[e.b].p],ids:[e.a,e.b],faces:e.faces,microEdgeWelds,beforePairs}));}const[a,b]=e.faces;
 ...
 const regionOwners=[['proximal anterior brachialis return','proximal brachialis support','proximal posterior brachialis return'],['anterior brachialis facing','brachialis crown','posterior brachialis facing'],['distal anterior brachialis return','brachialis elbow insertion','distal posterior brachialis return']];
 for(const r of registry.values()){
  ...
  if(residual>2e-7||spread>.2)throw new Error('M127 facing family is not coplanar '+JSON.stringify({id:r.id,residual,spread,area,triangles:r.triangleIndices}));
  const cell=r.id.match(/FLOW_(\d)_(\d)_/);r.anatomicalOwner=cell?regionOwners[Number(cell[1])][Number(cell[2])]:'lateral brachialis crown';
 ...
 g.userData.mrsCrystal.explicitGeometryPlanes={version:'M127',count:registry.size,ownership:'Final upper-arm physical triangle registry; facings follow the brachialis crown and support envelope.'};
 g.userData.mrsBrachialisPatch127={version:'M127',parent:'M126',reports,planes:[...registry.values()],creases,microEdgeWelds,oldToNewTriangle:mapping,oldToNewTriangles:children,oldTriangleCount:faces.length,triangleCount:final.length};
