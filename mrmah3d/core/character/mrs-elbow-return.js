...
import {elbowReturnData} from './mrs-elbow-return-data.js';

// M27: an implicit cotangent surface correction, measured on the retained
// M20 joined topology. Positions and normals are from the same actual mesh.
// Support is the proximal elbow return, ending at forearm t=.12; unchanged
// corners retain every attribute. No material or normal-only concealment.
export function refineMrsElbowReturn({limbs}){
 const meshes=['left','right'].flatMap(s=>['fore','elbow-knob'].map(p=>limbs[s].group.getObjectByName(`arm-${s}-${p}`)));
 for(const mesh of meshes){
  const g=mesh.geometry,spec=elbowReturnData[mesh.name],p=g.attributes.position;
  if(!g.userData.elbowUnion||p.count!==spec.corners)throw Error('M27 crease requires M27 source-face union topology');
  ...
  for(const f of changed){
   ...
   if(area===0)throw Error('M27 degenerate elbow triangle');cross.normalize();
  ...
  g.computeBoundingBox();g.computeBoundingSphere();g.userData.elbowReturn={parent:'R166-M26-9b90933fb295',method:'implicit cotangent geometric fairing; shared angle-weighted geometric normals',axialEnd:.12,maxDisplacement:.0067269632168802055};
