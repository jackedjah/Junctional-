...
export function authorMrsDeltoidSide143(g,arm,side){
 ...
 g.userData.mrsDeltoidSideBoundary143={version:'M143',oldToNewTriangles:Array.from({length:p.count/3},(_,i)=>[i]),originalTriangleCount:p.count/3,triangleCount:p.count/3,refinedBaselinePositions:Array.from(p.array),stations:null,method:'Existing shoulder topology retained; anatomical source-normal ownership sets the facing footprint'};
 const protectedKeys=new Set(['mrsBrachialisPatch127','mrsDeltoid128','mrsDeltoidInsertion129','mrsBiceps139','mrsBicepsProximal141','mrsDeltoidFront142'].flatMap(k=>g.userData[k].planes.flatMap(p=>p.triangleIndices.flatMap(t=>before.slice(t*3,t*3+3).map(key)))));
 const coords=v=>({t:v.dot(axis)/length,angle:Math.atan2(v.dot(out),v.dot(front))});
 const owners=[['DELTOID_LATERAL_RETURN',.70,'lateral deltoid lower return']];
 const fits=owners.map(([owner,angle,anatomicalOwner])=>{
  const guide=front.clone().multiplyScalar(Math.cos(angle)).addScaledVector(out,Math.sin(angle)).addScaledVector(axis,.20).normalize(),eligible=[],edges=new Map();
  for(let i=0;i<p.count;i+=3){const v=before.slice(i,i+3),q=v.map(coords),cross=v[1].clone().sub(v[0]).cross(v[2].clone().sub(v[0])),area2=cross.length(),normal=cross.clone().normalize();
   if(v.some(p=>protectedKeys.has(key(p)))||!q.every(c=>c.t>=.03&&c.t<=.28)||q.reduce((s,c)=>s+c.t,0)/3<.08||q.reduce((s,c)=>s+c.t,0)/3>.22||q.reduce((s,c)=>s+c.angle,0)/3<.65||q.reduce((s,c)=>s+c.angle,0)/3>1.15||normal.dot(guide)<.985)continue;
  ...
  groups.sort((a,b)=>b.reduce((s,v)=>s+v.area2,0)-a.reduce((s,v)=>s+v.area2,0));let core=groups[0];if(!core||core.length<2)throw new Error('M143 missing connected '+side+' '+owner+' crown');
  ...
  const fixedFaces=[];for(let i=0;i<p.count;i+=3){const v=before.slice(i,i+3);if(v.some(q=>keys.has(key(q)))&&v.every(q=>keys.has(key(q))||protectedKeys.has(key(q))||coords(q).t<=.02||coords(q).t>=.30)){const cross=v[1].clone().sub(v[0]).cross(v[2].clone().sub(v[0]));fixedFaces.push({v,normal:cross.clone().normalize(),area:cross.length()});}}
  ...
  best ||= measure(angle,.2);if(!best)for(let i=-10;i<=10;i++)for(let j=-8;j<=22;j++){const fit=measure(angle+i*.05,j*.05);if(fit&&(!best||fit.distance<best.distance))best=fit;}if(!best)throw new Error('M143 invalid crown chart '+owner);let radius=.24;
  ...
  if(best.distance>.008)throw new Error('M143 crown would erase retained mass '+side+' '+owner+' '+best.distance);
 ...
 const owners=coreOwner.get(key(before[i]));if(!owners)continue;const v=before[i];
  if(owners.length===1){const fit=owners[0],radial=fit.normal.clone().addScaledVector(axis,-fit.normal.dot(axis)).normalize();target[i].addScaledVector(radial,(fit.offset-v.dot(fit.normal))/radial.dot(fit.normal));}
   ...
   if(Math.abs(det)<.1)throw new Error('M143 unresolved ridge ownership');target[i].addScaledVector(out,(da*bz-db*az)/det).addScaledVector(front,(ax*db-bx*da)/det);}
  if(target[i].distanceTo(v)>.008)throw new Error('M143 excessive shared crown displacement '+side+' '+target[i].distanceTo(v));
 }
 const near=v=>Math.min(...fits.flatMap(q=>q.vertices.map(p=>v.distanceTo(p)))),start=.02;
 ...
 for(let stage=1;stage<=12;stage++){const map=new Map(current.map((v,i)=>[key(v),i])),stageTarget=current.map((v,i)=>coreKeys.has(key(before[i]))?before[i].clone().lerp(target[i],stage/12):v.clone());
  try{solve=coupleFacingReturns(current,stageTarget,v=>{const i=map.get(key(v));return coreKeys.has(key(before[i]))||protectedKeys.has(key(before[i]));},v=>{const q=before[map.get(key(v))],t=coords(q).t;return t>start&&t<.30&&near(q)<.035;},before,{normalCone:.90,lambda:.000065});}catch(e){throw new Error('M143 continuation stage '+stage+': '+e.message);}
 ...
 if(minArea<.60||minDot<.89)throw new Error('M143 invalid physical support '+JSON.stringify({side,minArea,minDot}));
 ...
 g.userData.mrsDeltoidSide143={version:'M143',parent:'M142',side,frame:f,method:'Connected source-normal-compatible lower lateral-deltoid facing, preserving the existing shoulder crowns, lateral insertion and biceps system',planes,changedCopies:changed,maximumDisplacement:maxMove,minimumAreaRatio:minArea,minimumNormalDot:minDot,returnSolve:{freeNodes:solve.freeNodes,coreNodes:solve.coreNodes,...solve.orientationProjection},normalCorners,continuation:solve.continuation,protected:'All retained brachialis/deltoid facings, shoulder and elbow seats, forearms and other meshes; upper-arm silhouette measured independently',status:'candidate; actual geometry and views required'};
