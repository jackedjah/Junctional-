...
export function shapeMrsObliqueIntegration(g){
 ...
 const center=.135+(y-1.57)*.25,q=((x-center)/.072)**2+((y-1.715)/.165)**2;
 if(q>=1||z<=.07)continue;
 const dz=.020*(1-q)**3*ease((z-.07)/.065);p.setZ(i,z+dz);
 if(p.getZ(i)===z)continue;maxMove=Math.max(maxMove,Math.abs(p.getZ(i)-z));changedCorners++;edited.add(Math.floor(i/3));
 }
 refresh(g,edited);const meta={parent:'R166-M32-d99f3c60f1fc',method:'Diagonal oblique support volume; fixed X/Y, protected waist outline',maxMove,changedCorners};g.userData.mrsObliqueIntegration=meta;return meta;
}
