...
export var TORSO = {
  ...
  rings: [
    ...
    /* THE LONG DESCENT: from the apex the quad falls away on a convex
       ...
       R232: the sweep begins to insert on this row (0.22 -> 0.12) so the
...
export const MRMAH_MORPHOLOGY = {
  ...
  LOWER_BACK:[[0,0],[.20,.025],[.43,.060],[.62,.092],[.70,.116],[.78,.124],[.88,.142],[.98,.1560],[1.08,.1660],[1.25,.2020],[1.36,.1880],[1.48,.1520],[1.56,.1549]],
  LOWER_PATHS:{
...
/* R239 — THE ASTRA R109 ANTERIOR QUAD, RESTORED.

   These are `thighShape`'s own lobe geometry and its own per-row coefficients,
   taken verbatim from the R109 Astra checkpoint (commit 0c05b31,
   proportions.js `thighShape` and the lower ring rows at y 0.870 to 1.410),
   with the posterior terms (glute, cleft, ham, hamCleft) deliberately left out
   because this restoration is anterior only. `depth` is Astra's own `d` for
   those rows. Angles are radians from the FRONT SEAM, which is the convention
   `torsoSurface` already uses (front = max(0, sin(a))), so the lobes need no
   re-basing.

   Why this exists: measured on the two built meshes at matched heights, the
   Astra anterior section runs 0.152 at the seam to 0.259 at the quad crown, a
   rise of 70%, while the R223-lineage surface that replaced it runs 0.208 to
   0.239, a rise of 15%. That single number is the whole of "a flat shield with
...
MRMAH_MORPHOLOGY.lower.astraQuad={
 ...
 /* R253 — the region starts at 0.58, not 0.66. At 0.66 the window's own fade
    made the weight ZERO there, so every distal knot added in R252 was dead code
    and the anterior below the knee was still pure LOWER_FRONT. */
 region:[0.58,1.46,0.10], sideFade:0.06, projection:0.870,
 lobes:{rf:[0.44,0.64],vl:[0.98,0.50],vm:[0.26,0.30],valley:[0.74,0.28],itb:[1.95,0.30],add:[0,0.50]},
 depth:[[0.58,0.1280],[0.66,0.1450],[0.72,0.1520],[0.80,0.1562],[0.87,0.1485],[0.95,0.1473],[1.03,0.1600],[1.10,0.1680],[1.23,0.1620],[1.32,0.1640],[1.41,0.1560],[1.46,0.1640]],
 rf:[[0.58,0],[0.66,0.02],[0.72,0.06],[0.80,0.11],[0.87,0.18],[0.95,0.38],[1.03,0.48],[1.10,0.52],[1.23,0.46],[1.32,0.33],[1.41,0.20],[1.46,0.11]],
 vl:[[0.58,0],[0.66,0.02],[0.72,0.07],[0.80,0.13],[0.87,0.26],[0.95,0.44],[1.03,0.54],[1.10,0.62],[1.23,0.66],[1.32,0.64],[1.41,0.48],[1.46,0.30]],
 vm:[[0.58,0],[0.66,0.04],[0.72,0.11],[0.80,0.20],[0.87,0.30],[0.95,0.32],[1.03,0.23],[1.10,0.14],[1.23,0.10],[1.32,0.08],[1.41,0.05],[1.46,0]],
 valley:[[0.58,0],[0.66,0],[0.72,0.02],[0.80,0.04],[0.87,0.07],[0.95,0.15],[1.03,0.20],[1.10,0.22],[1.23,0.21],[1.32,0.15],[1.41,0.07],[1.46,0.03]],
 itb:[[0.58,0],[0.66,0],[0.80,0.02],[0.87,0.04],[0.95,0.06],[1.03,0.06],[1.10,0.06],[1.23,0.05],[1.32,0],[1.46,0]],
 add:[[0.58,0],[0.66,0],[0.72,0.015],[0.80,0.028],[0.87,0.038],[0.95,0.045],[1.03,0.050],[1.10,0.050],[1.23,0.042],[1.32,0.028],[1.41,0.014],[1.46,0]]};
