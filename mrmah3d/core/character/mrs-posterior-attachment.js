...
export function authorMrsPosteriorAttachment146(g,arm,side){
 ...
 g.userData.mrsPosteriorBoundary146={version:'M146',oldToNewTriangles:Array.from({length:p.count/3},(_,i)=>[i]),originalTriangleCount:p.count/3,triangleCount:p.count/3,refinedBaselinePositions:Array.from(p.array),stations:null,method:'Existing shoulder topology retained; anatomical source-normal ownership sets the facing footprint'};
 const protectedKeys=new Set(['mrsBrachialisPatch127','mrsDeltoid128','mrsDeltoidInsertion129','mrsBiceps139','mrsBicepsProximal141','mrsDeltoidFront142','mrsDeltoidSide143','mrsTriceps144'].flatMap(k=>g.userData[k].planes.flatMap(p=>p.triangleIndices.flatMap(t=>before.slice(t*3,t*3+3).map(key)))));
 const coords=v=>({t:v.dot(axis)/length,angle:(Math.atan2(v.dot(out),v.dot(front))+Math.PI*2)%(Math.PI*2)});
 for(const v of before){const t=v.dot(axis)/length,a=Math.atan2(v.dot(out),v.dot(front));if(t>.20&&t<.69&&Math.abs(a-2.68)<.10)protectedKeys.add(key(v));}
 const owners=[
 ['DELTOID_POSTERIOR_RETURN',2.80,.50,[-.02,.15],[.01,.09],[2.60,3.22],'posterior deltoid descending insertion'],
 ['TRICEPS_PROXIMAL_RETURN',2.72,-.05,[.10,.29],[.14,.24],[2.55,3.20],'proximal triceps return below posterior deltoid']
 ];
 const fits=owners.map(([owner,angle,slopeSeed,extent,centerRange,angleRange,anatomicalOwner])=>{
  const guide=front.clone().multiplyScalar(Math.cos(angle)).addScaledVector(out,Math.sin(angle)).addScaledVector(axis,slopeSeed).normalize(),eligible=[],edges=new Map();
  for(let i=0;i<p.count;i+=3){const v=before.slice(i,i+3),q=v.map(coords),cross=v[1].clone().sub(v[0]).cross(v[2].clone().sub(v[0])),area2=cross.length(),normal=cross.clone().normalize();
   if(v.some(p=>protectedKeys.has(key(p)))||!q.every(c=>c.t>=extent[0]&&c.t<=extent[1])||q.reduce((s,c)=>s+c.t,0)/3<centerRange[0]||q.reduce((s,c)=>s+c.t,0)/3>centerRange[1]||q.reduce((s,c)=>s+c.angle,0)/3<angleRange[0]||q.reduce((s,c)=>s+c.angle,0)/3>angleRange[1]||normal.dot(guide)<.985)continue;
  ...
  groups.sort((a,b)=>b.reduce((s,v)=>s+v.area2,0)-a.reduce((s,v)=>s+v.area2,0));let core=groups[0];if(!core||core.length<2)throw new Error('M146 missing connected '+side+' '+owner+' crown');
  // The paired retained source charts differ in tessellation. Bound their
  // transverse footprint in physical arm units before fitting a plane.
  // This corrects ownership width without rescaling the muscle or its groove.
  if(owner==='TRICEPS_PROXIMAL_RETURN'){
   const transverse=guide.clone().cross(axis).normalize(),u=core.flatMap(f=>f.v.map(v=>v.dot(transverse))),mid=(Math.min(...u)+Math.max(...u))/2;
   core=core.filter(f=>f.v.every(v=>Math.abs(v.dot(transverse)-mid)<=.02825));
   if(core.length<2)throw new Error('M146 missing bounded proximal return '+side);
  }
  ...
  const fixedFaces=[];for(let i=0;i<p.count;i+=3){const v=before.slice(i,i+3);if(v.some(q=>keys.has(key(q)))&&v.every(q=>keys.has(key(q))||protectedKeys.has(key(q))||coords(q).t<=-.04||coords(q).t>=.30)){const cross=v[1].clone().sub(v[0]).cross(v[2].clone().sub(v[0]));fixedFaces.push({v,normal:cross.clone().normalize(),area:cross.length()});}}
  ...
  best ||= measure(angle,slopeSeed);if(!best)for(let i=-10;i<=10;i++)for(let j=-8;j<=22;j++){const fit=measure(angle+i*.05,j*.05);if(fit&&(!best||fit.distance<best.distance))best=fit;}if(!best)throw new Error('M146 invalid crown chart '+owner);let radius=.24;
  ...
  if(best.distance>.008)throw new Error('M146 crown would erase retained mass '+side+' '+owner+' '+best.distance);
 ...
 const owners=coreOwner.get(key(before[i]));if(!owners)continue;const v=before[i];
  if(owners.length===1){const fit=owners[0],radial=fit.normal.clone().addScaledVector(axis,-fit.normal.dot(axis)).normalize();target[i].addScaledVector(radial,(fit.offset-v.dot(fit.normal))/radial.dot(fit.normal));}
   ...
   if(Math.abs(det)<.1)throw new Error('M146 unresolved ridge ownership');target[i].addScaledVector(out,(da*bz-db*az)/det).addScaledVector(front,(ax*db-bx*da)/det);}
  if(target[i].distanceTo(v)>.008)throw new Error('M146 excessive shared crown displacement '+side+' '+target[i].distanceTo(v));
 }
 const near=v=>Math.min(...fits.flatMap(q=>q.vertices.map(p=>v.distanceTo(p)))),start=-.04;
 ...
 for(let stage=1;stage<=12;stage++){const map=new Map(current.map((v,i)=>[key(v),i])),stageTarget=current.map((v,i)=>coreKeys.has(key(before[i]))?before[i].clone().lerp(target[i],stage/12):v.clone());
  try{solve=coupleFacingReturns(current,stageTarget,v=>{const i=map.get(key(v));return coreKeys.has(key(before[i]))||protectedKeys.has(key(before[i]));},v=>{const q=before[map.get(key(v))],t=coords(q).t;return t>start&&t<.30&&near(q)<.035;},before,{normalCone:.90,lambda:.000065,stableTopology:true});}catch(e){throw new Error('M146 continuation stage '+stage+': '+e.message);}
 ...
 if(minArea<.60||minDot<.89)throw new Error('M146 invalid physical support '+JSON.stringify({side,minArea,minDot}));
 ...
 g.userData.mrsPosterior146={version:'M146',parent:'M145',side,frame:f,method:'Paired source-normal-compatible posterior-deltoid insertion and proximal triceps return; original humeral frame and fixed crown boundaries',planes,controls:{maximumProximalTransverseWidth:.0565,domain:[-.04,.30],owners:owners.map(([owner,angle,slope,extent,centerRange,angleRange])=>({owner,angle,slope,extent,centerRange,angleRange}))},changedCopies:changed,maximumDisplacement:maxMove,minimumAreaRatio:minArea,minimumNormalDot:minDot,returnSolve:{freeNodes:solve.freeNodes,coreNodes:solve.coreNodes,...solve.orientationProjection},normalCorners,continuation:solve.continuation,protected:'All retained brachialis/deltoid facings, shoulder and elbow seats, forearms and other meshes; upper-arm silhouette measured independently',status:'candidate; actual geometry and views required'};
