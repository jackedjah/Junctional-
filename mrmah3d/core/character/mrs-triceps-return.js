...
export function continueMrsTricepsReturn145(g,arm,side){
 ...
 const planes=g.userData.mrsTriceps144.planes,protectedPlanes=['mrsBrachialisPatch127','mrsDeltoid128','mrsDeltoidInsertion129','mrsBiceps139','mrsBicepsProximal141','mrsDeltoidFront142','mrsDeltoidSide143','mrsTriceps144'].flatMap(k=>g.userData[k].planes);
 const protectedKeys=new Set(protectedPlanes.flatMap(p=>p.triangleIndices.flatMap(t=>stock.slice(t*3,t*3+3).map(key))));
 const out=new Vector3().fromArray(frame.out),front=new Vector3().fromArray(frame.front);
 for(const v of stock){const t=v.dot(axis)/length,a=Math.atan2(v.dot(out),v.dot(front));if(t>.20&&t<.69&&Math.abs(a-2.68)<.10)protectedKeys.add(key(v));}
 const edges=new Map();
 ...
 for(const pl of planes){const ids=new Set(pl.triangleIndices),normal=new Vector3().fromArray(pl.averageNormal),center=new Vector3().fromArray(pl.centerPosition);
  for(const e of edges.values())if(e.triangles.some(t=>ids.has(t))&&e.triangles.some(t=>!core.has(t))&&(e.a.dot(axis)+e.b.dot(axis))/(2*length)>Math.max(.44,center.dot(axis)/length+.025))
   boundary.push({...e,line:new Line3(e.a,e.b),normal,center,owner:pl.id});
 }
 if(!boundary.length)throw new Error('M145 missing actual distal facing boundary');
 const near=v=>Math.min(...boundary.map(e=>e.line.closestPointToPoint(v,true,new Vector3()).distanceTo(v)));
 const domain=v=>{const t=v.dot(axis)/length;return t>.44&&t<.77&&near(v)<.035;};
 const targets=stock.map(v=>{
  ...
  if(!total)return v.clone();return v.clone().addScaledVector(radial,sum/total*weight*ease((t-.44)/.045)*(1-ease((t-.69)/.08)));
 });
 const solve=coupleFacingReturns(stock,targets,v=>protectedKeys.has(key(v)),domain,stock,{normalCone:.90,lambda:.00004,stableTopology:true});
 ...
 if(minimumNormalDot<.89||minimumAreaRatio<.60||maximumDisplacement>.008)throw new Error('M145 invalid distal support '+JSON.stringify({side,minimumNormalDot,minimumAreaRatio,maximumDisplacement}));
 ...
 g.userData.mrsTricepsReturn145={version:'M145',parent:'M144',side,frame,maximumDisplacement,minimumNormalDot,minimumAreaRatio,returns,weightedReturnAngleBefore:mean('beforeDegrees'),weightedReturnAngleAfter:mean('afterDegrees'),returnSolve:{freeNodes:solve.freeNodes,coreNodes:solve.coreNodes,...solve.orientationProjection},method:'Actual distal boundary continuation with fixed physical crowns and a coupled anatomical return skin',controls:{domain:[.44,.77],supportDistance:.035},protected:'All retained M127-M144 authored facing buffers and central inter-head rail, joint seats, forearms and other meshes'};
