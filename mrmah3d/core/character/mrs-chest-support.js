...
// Supported anterior volume over the retained chest. Sternum, lateral root,
// upper clavicle and lower abdominal attachment remain fixed.
export function shapeMrsChestSupport(g){
 ...
 for(let i=0;i<p.count;i++){
  ...
  if(z<=.08||y<=1.80||y>=2.16||x<=.025||x>=.34)continue;
  const q=((x-.174)/.168)**2+((y-1.978)/.149)**2;
  const weight=.70*ease((1-q)/.38)*ease((x-.025)/.055)*ease((z-.15)/.12);
  const target=.165+.255*Math.pow(Math.max(0,1-q),.65);
  const dz=(target-z)*weight;
  if(!dz)continue;p.setZ(i,z+dz);if(p.getZ(i)===z)continue;
  maxMove=Math.max(maxMove,Math.abs(p.getZ(i)-z));changedCorners++;edited.add(Math.floor(i/3));
 }
 refresh(g,edited);const meta={parent:'R166-M30-81143aaccbce',method:'Regional supported chest belly projection',maxMove,changedCorners};g.userData.mrsChestSupport=meta;return meta;
}
