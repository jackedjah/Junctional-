...
// Two continuous longitudinal owners: broad radial belly, narrowing tendon.
// The boundary is cut into retained triangles before the physical-plane solve.
export function rebuildMrsRadialBoundary135(g,arm,side){
 ...
 const protectedTriangles=new Set(g.userData.mrsForearmCrowns133.planes.flatMap(p=>p.triangleIndices));
 const protectedKeys=new Set([...protectedTriangles].flatMap(t=>stock.slice(t*3,t*3+3).map(key)));
 const active=v=>{const q=xyz(v);return q.z>.025&&q.t>.365+1e-7&&q.t<.82-1e-7&&Math.abs(q.x)<.047;};
 const intersecting=vs=>{const q=vs.map(xyz);return q.every(p=>p.z>.025)&&Math.min(...q.map(p=>p.t))<.82&&Math.max(...q.map(p=>p.t))>.365&&Math.min(...q.map(p=>p.x))<.047&&Math.max(...q.map(p=>p.x))>-.047;};
 const cut=fn=>{
  const edges=new Map();
  for(const f of faces){const vs=f.ids.map(i=>nodes[i].p);if(!intersecting(vs))continue;
   for(let k=0;k<3;k++){
    ...
    const v=pa.clone().lerp(pb,t),q=xyz(v);if(q.z<=.025||q.t<.365-1e-7||q.t>.82+1e-7||Math.abs(q.x)>.047)continue;
  ...
  for(const f of faces){const v=f.ids,m=v.map((a,k)=>edges.get([a,v[(k+1)%3]].sort((a,b)=>a-b).join(':'))),count=m.filter(i=>i!==undefined).length;
   ...
   if(protectedTriangles.has(f.parent))throw new Error('M135 boundary would split retained crown '+side+' '+f.parent);
 ...
 // Axial stations separate the belly from its distal insertion. Side boundaries
 // are open longitudinal tapers rather than a concentric diamond stamp.
 for(const t of [.365,.42,.46,.62,.76,.80,.82])cut(q=>q.t-t);
 const width=t=>t<=.62?.024-.006*(t-.46)/.16:.018-.009*(t-.62)/.14;
 for(const sign of [-1,1]){
  for(const extra of [0,.012,.020]){cut(q=>sign*q.x-(.024-.006*(q.t-.46)/.16+extra));cut(q=>sign*q.x-(.018-.009*(q.t-.62)/.14+extra));}
 }
 ...
 const crown=(q)=>q.z>.025&&q.t>=.46-1e-7&&q.t<=.62+1e-7&&Math.abs(q.x)<=width(q.t)+1e-7;
 const tendon=(q)=>q.z>.025&&q.t>=.62-1e-7&&q.t<=.76+1e-7&&Math.abs(q.x)<=width(q.t)+1e-7;
 // Section-fitted axial flow, with paired crown-direction spread capped below
 // eight degrees. The left distal return is flat rather than re-inflating.
 const offset=side==='left'?.06617528713731642:.0634988756963888;
 const crownSlope=side==='left'?-.0792570291474059:.0507429708525941;
 const tendonSlope=side==='left'?0:-.03893460986155739;
 const height=q=>offset+.15*q.x+crownSlope*(Math.min(q.y,.62*length)-.54*length)+tendonSlope*Math.max(0,q.y-.62*length);
 const targets=expanded.map(v=>{
  const q=xyz(v);if(!active(v)||protectedKeys.has(key(v)))return v.clone();
  const ax=ease((q.t-.365)/(.46-.365))*(1-ease((q.t-.76)/(.82-.76)));
  const lateral=1-ease((Math.abs(q.x)-width(q.t))/.020);
  return v.clone().addScaledVector(out,(height(q)-q.z)*ax*lateral);
 });
 const solve=coupleFacingReturns(expanded,targets,v=>protectedKeys.has(key(v))||crown(xyz(v))||tendon(xyz(v)),active,expanded,{normalCone:.90,lambda:.00004});
 ...
 if(minimumNormalDot<.89||minimumAreaRatio<.60||maximumDisplacement>.015)throw new Error('M135 invalid rebuilt support '+JSON.stringify({side,minimumNormalDot,minimumAreaRatio,maximumDisplacement}));
 ...
 final.forEach((f,ti)=>{
  ...
  let owner=null;if(vs.every(v=>crown(xyz(v))))owner='CROWN';else if(vs.every(v=>tendon(xyz(v))))owner='TENDON';if(owner)owners.set(ti,owner);
 ...
 for(const owner of ['CROWN','TENDON']){
  const triangles=[...owners].filter(([,v])=>v===owner).map(([i])=>i);if(!triangles.length)throw new Error('M135 missing physical '+owner);
  ...
  if(residual>2e-7||spread>.2)throw new Error('M135 non-planar '+owner+' '+JSON.stringify({residual,spread}));
  const id='MRS_FOREARM_'+lr+'_135_RADIAL_'+owner;
  planes.push({id,planeId:id,region:'FOREARM_RADIAL',side:lr,type:owner==='CROWN'?'long trapezoid':'narrow insertion wedge',anatomicalOwner:owner==='CROWN'?'brachioradialis belly':'radial distal tendon',triangleIndices:triangles,centerPosition:center.toArray(),averageNormal:normal.toArray(),apexDirection:axis.toArray(),area,maximumPlaneResidual:residual,maximumPhysicalNormalSpreadDegrees:spread,mirroredId:id.replace('_'+lr+'_','_'+(lr==='L'?'R':'L')+'_'),adjacentPlanes:['MRS_FOREARM_'+lr+'_135_RADIAL_'+(owner==='CROWN'?'TENDON':'CROWN')],blackPointAdjacency:[],status:'candidate; actual proof required'});
 }
 ...
 for(const ids of groups.values()){
  const v=moved[ids[0]],t=xyz(v).t;if(t<=.365||t>=.82||ids.some(i=>protectedKeys.has(key(expanded[i]))))continue;
 ...
 g.userData.mrsRadialBoundary135={version:'M135',parent:'M134',side,frame,planes,oldToNewTriangles:mapping,
  ...
  controls:{crown:[.46,.62],tendon:[.62,.76],widthAtCrownStart:.048,widthAtInsertion:.036,widthAtTendonEnd:.018,offset,lateralSlope:.15,crownSlope,tendonSlope,outerDomain:[.365,.82],returnWidth:.020},
  method:'Explicit axial and taper-boundary cuts; two physical owners; coupled return solve with retained orientation cones',
  returnSolve:{freeNodes:solve.freeNodes,coreNodes:solve.coreNodes,...solve.orientationProjection},protected:'M133 physical crowns and elbow/wrist seats; all other meshes'};
