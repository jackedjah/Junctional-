import {cutMrsBicepsStations139} from './mrs-biceps-boundary.js';
...
export function authorMrsBiceps139(g,arm,side){
 cutMrsBicepsStations139(g,side);
 ...
 const protectedKeys=new Set(['mrsBrachialisPatch127','mrsDeltoid128','mrsDeltoidInsertion129'].flatMap(k=>g.userData[k].planes.flatMap(p=>p.triangleIndices.flatMap(t=>before.slice(t*3,t*3+3).map(key)))));
 const coords=v=>({t:v.dot(axis)/length,angle:Math.atan2(v.dot(out),v.dot(front))});
 const owners=[['BICEPS_MEDIAL',-.70,'biceps medial crown facing'],['BICEPS_ANTERIOR',.18,'biceps anterior crown facing']];
 const fits=owners.map(([owner,angle,anatomicalOwner])=>{
  ...
  for(let i=0;i<p.count;i+=3){const v=before.slice(i,i+3),q=v.map(coords),cross=v[1].clone().sub(v[0]).cross(v[2].clone().sub(v[0])),area2=cross.length(),normal=cross.clone().normalize();
   if(v.some(p=>protectedKeys.has(key(p)))||!q.every(c=>c.t>=.38-1e-7&&c.t<=.54+1e-7)||q.reduce((s,c)=>s+c.angle,0)/3<-.8||q.reduce((s,c)=>s+c.angle,0)/3>.30||normal.dot(guide)<.94)continue;
  ...
  groups.sort((a,b)=>b.reduce((s,v)=>s+v.area2,0)-a.reduce((s,v)=>s+v.area2,0));const core=groups[0];if(!core||core.length<2)throw new Error('M139 missing connected '+side+' '+owner+' crown');
  ...
  best ||= measure(angle,0);if(!best)throw new Error('M139 invalid crown chart '+owner);let radius=.24;
  ...
  if(best.distance>.008)throw new Error('M139 crown would erase retained mass '+side+' '+owner+' '+best.distance);
 ...
 for(let i=0;i<before.length;i++){
  ...
  if(owners.length===1){const fit=owners[0],radial=v.clone().addScaledVector(axis,-v.dot(axis)).normalize();target[i].addScaledVector(radial,(fit.offset-v.dot(fit.normal))/radial.dot(fit.normal));}
   ...
   if(Math.abs(det)<.1)throw new Error('M139 unresolved ridge ownership');target[i].addScaledVector(out,(da*bz-db*az)/det).addScaledVector(front,(ax*db-bx*da)/det);}
  if(target[i].distanceTo(v)>.008)throw new Error('M139 excessive shared crown displacement '+side+' '+target[i].distanceTo(v));
 }
 const near=v=>Math.min(...fits.flatMap(q=>q.vertices.map(p=>v.distanceTo(p)))),start=.24;
 const solve=coupleFacingReturns(before,target,v=>coreKeys.has(key(v))||protectedKeys.has(key(v)),v=>{const t=coords(v).t;return t>start&&t<.74&&near(v)<.035;},before,{normalCone:.90,lambda:.000065});
 ...
 if(minArea<.60||minDot<.89)throw new Error('M139 invalid physical support '+JSON.stringify({side,minArea,minDot}));
 ...
 g.userData.mrsBiceps139={version:'M139',parent:'M137',side,frame:f,method:'Source-normal-compatible connected biceps crowns; shared anatomical ridge constrained to both physical planes; coupled retained return skin',planes,changedCopies:changed,maximumDisplacement:maxMove,minimumAreaRatio:minArea,minimumNormalDot:minDot,returnSolve:{freeNodes:solve.freeNodes,coreNodes:solve.coreNodes,...solve.orientationProjection},normalCorners,protected:'All retained brachialis/deltoid facings, shoulder and elbow seats, forearms and other meshes; upper-arm silhouette measured independently',status:'candidate; actual geometry and views required'};
